...
void Com_ExecuteCfg(void)
{
	...
	if(!Com_SafeMode())
	{
		...
		if (g_update4firststartup->integer > 0)//playing academy
		{
			//
		}
...
void Com_Init( char *commandLine ) {
	...
	try {
		...
		g_update4firststartup = Cvar_Get("g_update4firststartup", "1", 0);
...
void Com_WriteConfigToFile( const char *filename ) {
	...
	FS_Printf (f, "// generated by MovieDuels-SaberBlock-Addon, do not modify\n");
	Key_WriteBindings (f);
