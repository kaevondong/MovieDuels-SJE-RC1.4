...
static void CG_PlayerAnimation( centity_t *cent, int *legsOld, int *legs, float *legsBackLerp,
	...
	qboolean HoldingBlock = (cent->currentState.ManualBlockingFlags & (1 << MBF_BLOCKING) ? qtrue : qfalse);
	qboolean ActiveBlocking = (cent->currentState.ManualBlockingFlags & (1 << MBF_PROJBLOCKING) ? qtrue : qfalse);
	...
	*torsoBackLerp = cent->pe.torso.backlerp;
	...
	if( newTorsoFrame )
	{
		...
		if (!PM_InKataAnim(cent->currentState.torsoAnim))
		{//make smooth animations			
			if ((ActiveBlocking && cent->currentState.saberMove == LS_READY))
			{
				speedScale *= 0.3f;
			}
		}
	...
	case AEV_EFFECT:
		...
		{//invalid effect
			...
			{//some sort of hard-coded effect
				...
				{
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.clientNum, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/push_md.mp3"));
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.clientNum, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/push.mp3"));
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.clientNum, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/pushlow.mp3"));
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.clientNum, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/pushhard.mp3"));
...
static void CG_G2PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t angles, int ManualBlockingFlags)
{
...
static void CG_PlayerPowerups( centity_t *cent )
{
	...
	if (cg_SerenityJediEngineMode.integer == 2
		...
		&& cent->gent->health > 1
		&& (cent->currentState.powerups & (1 << PW_FORCE_PUSH) || cent->currentState.powerups & (1 << PW_FORCE_PUSH_RHAND)))
	{
		switch (faction)
		{
		case FACTION_DARK:
	...
	//if ((cent->currentState.powerups & (1 << PW_BATTLESUIT)) && cent->gent->client->ps.weapon != WP_SABER)
	//{
...
#define REPULSE_EFFECT_DURATION		1000
...
static void CG_ForcePushRefraction(vec3_t org, centity_t* cent, float scaleFactor, vec3_t colour)
{
	...
	if ((cent->gent->client->ps.weapon == WP_NONE || cent->gent->client->ps.weapon == WP_MELEE)
		&& cent->gent->client->ps.forcePowersActive & (1 << FP_PUSH) && cent->gent->client->ps.groundEntityNum == ENTITYNUM_NONE)
	{
		scale = 1.0f;
	}
...
static void CG_ForceRepulseRefraction(vec3_t org, centity_t* cent, float scaleFactor, vec3_t colour)
{
	refEntity_t ent;
	vec3_t ang;
	float scale;
...
void CG_AddRefEntityWithPowerups( refEntity_t *ent, int powerups, centity_t *cent )
{
	...
	//------------------------------------------------------
	if ((!DoingDashAction) && (!IsPressingDashButton(cent->gent)) && (!(gent->client->ps.userInt3 & (1 << FLAG_DASHING)))
		...
		&& (gent->s.number || cg.renderingThirdPerson)
		&& (!(cent->gent->client->ps.legsAnim == BOTH_DASH_B
			|| cent->gent->client->ps.legsAnim == BOTH_DASH_F
			|| cent->gent->client->ps.legsAnim == BOTH_DASH_R
			|| cent->gent->client->ps.legsAnim == BOTH_DASH_L
			|| cent->gent->client->ps.legsAnim == BOTH_HOP_R
			|| cent->gent->client->ps.legsAnim == BOTH_HOP_L
			|| cent->gent->client->ps.legsAnim == BOTH_HOP_B
			|| cent->gent->client->ps.legsAnim == BOTH_HOP_F))) // looks dumb doing this with first peron mode on
	{
		//FIXME: debounce this
		localEntity_t	*ex;
...
*/
	...
	if (cg_SerenityJediEngineMode.integer && !in_camera)
	{
		if (cg_SaberInnonblockableAttackWarning.integer == 1)
		{
			...
			{
				if (PM_SaberInnonblockableAttack(cent->currentState.torsoAnim))
				{
					ent->renderfx |= RF_RGB_TINT;
					ent->shaderRGBA[0] = 255;
					ent->shaderRGBA[1] = ent->shaderRGBA[2] = 0;
...
void CG_DrawPlayerShield(centity_t* cent, vec3_t origin)
{
	...
	alpha = 255.0f * ((cent->damageTime - cg.time) / MIN_SHIELD_TIME) + Q_flrand(0.1f, 1.0f) * 16;
	if (alpha > 255)
		alpha = 255;
...
void CG_DoOTSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
	...
	case SABER_UNSTABLE_RED:
		...
		break;
	default:
		glow = cgs.media.rgbSaberGlowShader = cgi_R_RegisterShader("gfx/effects/sabers_ot/ot_rgbGlow");
		blade = cgs.media.otSaberCoreShader;
		cgs.media.sfxSaberEndShader = cgi_R_RegisterShader("SFX_Sabers/saber_end");
		cgs.media.sfxSaberTrailShader = cgi_R_RegisterShader("SFX_Sabers/saber_trail");
		break;
...
void CG_DoRotJSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
...
void CG_DoTFASaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
...
void CG_DoUnstableSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
...
static void CG_DoSaberUnstable(vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	vec3_t		mid;
	qhandle_t	blade = 0, glow = 0;
	refEntity_t saber;
...
#define SABER_TRAIL_TIME	40.0f
		...
		{
			if (cent->gent->client->ps.saber[saberNum].type == SABER_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_UNSTABLE)
			{
				CG_DoSaberUnstable(org_, axis_[0], length, client->ps.saber[saberNum].blade[bladeNum].lengthMax, client->ps.saber[saberNum].blade[bladeNum].radius, client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)!noDlight);
			}
			else
			{
				CG_DoSaber(org_, axis_[0], length, client->ps.saber[saberNum].blade[bladeNum].lengthMax, client->ps.saber[saberNum].blade[bladeNum].radius, client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)!noDlight);
		...
		else
		{
			...
			else if (!(cent->gent->client->ps.saber[saberNum].type == SABER_SITH_SWORD || client->ps.saber[saberNum].saberFlags2 & SFL2_NO_BLADE))
			{
				...
				case 1:
					if (cent->gent->client->ps.saber[saberNum].type == SABER_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_UNSTABLE)
					{
						CG_DoUnstableSaber(fx->mVerts[0].origin, fx->mVerts[1].origin, fx->mVerts[2].origin, fx->mVerts[3].origin, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
					}
					else
					{
				...
				case 6:
					...
					{
						CG_DoRotJSaber(fx->mVerts[0].origin, fx->mVerts[1].origin, fx->mVerts[2].origin, fx->mVerts[3].origin, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(client->ps.saber[saberNum].numBlades < 3 && !(client->ps.saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)), (qboolean)(noDlight == qfalse), cent->currentState.clientNum, saberNum);
					}
					break;
				case 7:
					if (cent->gent->client->ps.saber[saberNum].type == SABER_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_UNSTABLE)
					...
					{
						CG_DoTFASaber(fx->mVerts[0].origin, fx->mVerts[1].origin, fx->mVerts[2].origin, fx->mVerts[3].origin, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(client->ps.saber[saberNum].numBlades < 3 && !(client->ps.saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)), (qboolean)(noDlight == qfalse), cent->currentState.clientNum, saberNum);
					}
					break;
				case 8:
					if (cent->gent->client->ps.saber[saberNum].type == SABER_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_UNSTABLE)
		...
		if ( cent->currentState.eFlags & EF_LOCKED_TO_WEAPON && cent->gent && cent->gent->health > 0 && cent->gent->owner )
		{
			...
			if ( chair && chair->gent )
			{
				...
				{//put behind it, not in chair
					...
					cent->lerpAngles[ROLL] = 0;
					CG_G2PlayerAngles( cent, ent.axis, tempAngles, cent->currentState.ManualBlockingFlags);
					calcedMp = qtrue;
				}
				else
				{//sitting in it
...
//HACK - add swoop model
	...
	SkipTrueView:
			...
			*/
			...
			{//doing the electrocuting
				...
				{//arc
					...
					if (cg_SerenityJediEngineMode.integer == 2)
					{
						theFxScheduler.PlayEffect(cgs.effects.forceLightningWide_md, cent->gent->client->renderInfo.handLPoint, fxAxis);
					}
					else
					{
					...
					{//jackin' 'em up, Palpatine-style
						if (cg_SerenityJediEngineMode.integer == 2)
						{
							theFxScheduler.PlayEffect(cgs.effects.forceLightningWide_md, cent->gent->client->renderInfo.handRPoint, fxAxis);
						}
						else
						{
				...
				{//line
					...
					if (cg_SerenityJediEngineMode.integer == 2)
					{
						theFxScheduler.PlayEffect(cgs.effects.forceLightning_md, cent->gent->client->renderInfo.handLPoint, fxDir);
					}
					else
					{
		...
		{
			if (cg_SerenityJediEngineMode.integer == 2 && (cent->gent->client->ps.weapon == WP_NONE || cent->gent->client->ps.weapon == WP_MELEE)
				&& cent->gent->client->ps.groundEntityNum == ENTITYNUM_NONE && cent->gent->client->NPC_class == CLASS_GALEN)
			{
				vec3_t blue;
