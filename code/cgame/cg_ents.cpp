...
static void CG_Missile( centity_t *cent ) {
	...
	else if (s1->powerups & (1 << PW_FORCE_PROJECTILE))
	{
		if (s1->weapon == WP_CONCUSSION)
		{
			...
			cgi_R_AddLightToScene(cent->lerpOrigin, 125, 1.0, 0.25, 0.75);
...
void CG_StasisEffect(centity_t* cent)
{
	...
	if (cent->currentState.boltInfo & MEF_LOOK_AT_ENEMY)
	{
		...
		float	STASIS_EFFECT_TIME_HALF = STASIS_EFFECT_TIME / 2.0f;
		float	X = 1.0f;
...
void CG_StasisEffectLong(centity_t* cent)
{
