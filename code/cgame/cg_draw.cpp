...
================
*/
static void CG_DrawForcePower_md(const centity_t* cent, const int xPos, const int yPos)
{
	int			i;
	...
	for (i = MAX_HUD_TICS - 1; i >= 0; i--)
	{
		...
		CG_DrawPic(forceTics_md[i].xPos,
			forceTics_md[i].yPos,
	...
	{
		cgi_R_SetColor(otherHUDBits[OHB_FORCEAMOUNT_MD].color);
	}

	// Print force numeric amount
	CG_DrawNumField(
		otherHUDBits[OHB_FORCEAMOUNT_MD].xPos,
		otherHUDBits[OHB_FORCEAMOUNT_MD].yPos,
		3,
		cent->gent->client->ps.forcePower,
		otherHUDBits[OHB_FORCEAMOUNT_MD].width,
		otherHUDBits[OHB_FORCEAMOUNT_MD].height,
		NUM_FONT_SMALL,
		qfalse);
}
...
================
*/
static void CG_DrawSaberStyle_md(const centity_t* cent, const int xPos, const int yPos)
{
	int index;
...
static void CG_DrawblockingMode(const centity_t* cent, const int xPos, const int yPos)
{
	int Blockindex;
...
================
*/
static void CG_Drawgunfatigue(const centity_t* cent, const int xPos, const int yPos)
{//render the CG_Drawgunfatigue meter.
	...
	inc = (float)BlasterAttackChainCount / MAX_HUD_TICS;
	value = cent->gent->client->ps.BlasterAttackChainCount;
...
static void CG_Drawsaberfatigue(const centity_t* cent, const int xPos, const int yPos)
{
	int			i;
	vec4_t		calcColor;
	float		value, inc, percent;
	...
	if (cg.mishapHUDTotalFlashTime > cg.time || (cent->gent->client->ps.saberAttackChainCount > MISHAPLEVEL_HUDFLASH))
	{
		...
		if (cg.mishapHUDNextFlashTime < cg.time)
		{
			...
			if (cent->gent->client->ps.saberAttackChainCount > MISHAPLEVEL_MAX)
			{
				cgi_S_StartSound(NULL, 0, CHAN_AUTO, cgs.media.overload);
			}
	...
	inc = (float)saberAttackChainCount / MAX_SJEHUD_TICS;
	value = cent->gent->client->ps.saberAttackChainCount;

	for (i = MAX_SJEHUD_TICS - 1; i >= 0; i--)
	{
		if (value <= 0)	// no more
		{
			break;
...
#define BPFUELBAR_W			15.0f
#define BPFUELBAR_X			(SCREEN_WIDTH-BPFUELBAR_W-8.0f)
#define BPFUELBAR_Y			250.0f
void CG_DrawoldblockPoints(const centity_t * cent)
{
	...
	percent = ((float)cg.snap->ps.blockPoints / 100.0f) * BPFUELBAR_H;
	...
	CG_FillRect(x + 1.0f, y + 1.0f + (BPFUELBAR_H - percent), BPFUELBAR_W - 1.0f, BPFUELBAR_H - 1.0f - (BPFUELBAR_H - percent), aColor);

	//then draw the other part greyed out
	CG_FillRect(x + 1.0f, y + 1.0f, BPFUELBAR_W - 1.0f, BPFUELBAR_H - percent, cColor);
}

static void CG_DrawblockPoints(int x, int y)
{
	vec4_t calcColor;
	float	blockPercent, hold;
	qboolean	flash = qfalse;
	...
	hold = cg.snap->ps.blockPoints - (BLOCK_POINTS_MAX / 2);
	blockPercent = (float)hold / (BLOCK_POINTS_MAX / 2);
	...
	{
		blockPercent = (float)cg.snap->ps.blockPoints / (BLOCK_POINTS_MAX / 2);
	}
	...
	CG_DrawNumField(
		otherHUDBits[OHB_BLOCKAMOUNT].xPos,
		otherHUDBits[OHB_BLOCKAMOUNT].yPos,
		3,
		cg.snap->ps.blockPoints,
		otherHUDBits[OHB_BLOCKAMOUNT].width,
...
//
//
//	percent = ((float)cg.snap->ps.cloakFuel / 100.0f) * CLFUELBAR_H;
...
================
*/
static void CG_DrawAmmo_md(const centity_t* cent, const int xPos, const int yPos)
{
	playerState_t* ps;
	int			i;
	vec4_t		calcColor;
	...
	else
	{
		if (currValue > 0)
		{
			...
			{
				memcpy(calcColor, otherHUDBits[OHB_AMMOAMOUNT_MD].color, sizeof(vec4_t));
			}
		}
		else
		{
	...
	CG_DrawNumField(
		otherHUDBits[OHB_AMMOAMOUNT_MD].xPos,
		otherHUDBits[OHB_AMMOAMOUNT_MD].yPos,
		3,
		ps->ammo[weaponData[cent->currentState.weapon].ammoIndex],
		otherHUDBits[OHB_AMMOAMOUNT_MD].width,
		otherHUDBits[OHB_AMMOAMOUNT_MD].height,
		NUM_FONT_SMALL,
		qfalse);
	...
	for (i = MAX_HUD_TICS - 1; i >= 0; i--)
	{
		...
		{
			memcpy(calcColor, ammoTics_md[i].color, sizeof(vec4_t));
			float percent = currValue / inc;
			calcColor[3] *= percent;
		}

		cgi_R_SetColor(calcColor);
		CG_DrawPic(ammoTics_md[i].xPos,
			ammoTics_md[i].yPos,
			ammoTics_md[i].width,
...
================
*/
static void CG_DrawHealth_md(const int x, const int y, const int w, const int h)
{
	vec4_t			calcColor;
	...
	for (i = (MAX_HUD_TICS - 1); i >= 0; i--)
	{
		...
		{
			memcpy(calcColor, healthTics_md[i].color, sizeof(vec4_t));
			float percent = currValue / inc;
	...
	{
		// Print force health amount
		cgi_R_SetColor(otherHUDBits[OHB_HEALTHAMOUNT_MD].color);

		CG_DrawNumField(
			otherHUDBits[OHB_HEALTHAMOUNT_MD].xPos,
			otherHUDBits[OHB_HEALTHAMOUNT_MD].yPos,
			3,
			ps->stats[STAT_HEALTH],
			otherHUDBits[OHB_HEALTHAMOUNT_MD].width,
			otherHUDBits[OHB_HEALTHAMOUNT_MD].height,
			NUM_FONT_SMALL,
			qfalse);
	}
	else
	{
		...
		CG_DrawNumField(
			otherHUDBits[OHB_HEALTHAMOUNTALT].xPos,
			otherHUDBits[OHB_HEALTHAMOUNTALT].yPos,
			3,
			ps->stats[STAT_HEALTH],
			otherHUDBits[OHB_HEALTHAMOUNTALT].width,
...
================
*/
static void CG_DrawArmor_md(const int x, const int y, const int w, const int h)
{
	vec4_t			calcColor;
	playerState_t* ps = &cg.snap->ps;
	...
	for (i = (MAX_HUD_TICS - 1); i >= 0; i--)
	{
		...
		{
			memcpy(calcColor, armorTics_md[i].color, sizeof(vec4_t));
			float percent = currValue / inc;
			calcColor[3] *= percent;
		}
	...
	if (cg.snap->ps.weapon == WP_SABER)
	{
		...
		{
			//Dont draw armour if saber is out and were drawing block points
		}
		else
		{	// Print armor amount
			cgi_R_SetColor(otherHUDBits[OHB_ARMORAMOUNT_MD].color);

			CG_DrawNumField(
				otherHUDBits[OHB_ARMORAMOUNT_MD].xPos,
				otherHUDBits[OHB_ARMORAMOUNT_MD].yPos,
				3,
				ps->stats[STAT_ARMOR],
				otherHUDBits[OHB_ARMORAMOUNT_MD].width,
				otherHUDBits[OHB_ARMORAMOUNT_MD].height,
				NUM_FONT_SMALL,
				qfalse);
		}
	}
...
static void CG_DrawHUD(centity_t* cent)
{
	...
	if (cgi_UI_GetMenuInfo("lefthud", &sectionXPos, &sectionYPos, &sectionWidth, &sectionHeight))
	{
		...
		if (cg_drawStatus.integer == 2)
		{
			...
			{
				CG_DrawSmallStringColor(sectionXPos + 5, sectionYPos - 60, va("Armor_md:%d", cg.snap->ps.stats[STAT_ARMOR]), colorTable[CT_HUD_GREEN]);
				CG_DrawSmallStringColor(sectionXPos + 5, sectionYPos - 40, va("Health_md:%d", cg.snap->ps.stats[STAT_HEALTH]), colorTable[CT_HUD_GREEN]);
			}
			else
			{
		...
		if (cg_SerenityJediEngineHudMode.integer == 1)
		{
			...
			CG_DrawPic(
				otherHUDBits[OHB_FRAME_LEFT_MD].xPos,
				otherHUDBits[OHB_FRAME_LEFT_MD].yPos,
				otherHUDBits[OHB_FRAME_LEFT_MD].width,
		...
		if (cg_SerenityJediEngineHudMode.integer == 1)
		{
			CG_DrawArmor_md(sectionXPos, sectionYPos, sectionWidth, sectionHeight);
			CG_DrawHealth_md(sectionXPos, sectionYPos, sectionWidth, sectionHeight);

			if (cg_SerenityJediEngineMode.integer == 2 && cent->currentState.weapon == WP_SABER)
			{
				CG_DrawblockPoints(x, y);
			}
		}
		else
		{
	...
	if (cgi_UI_GetMenuInfo("righthud", &sectionXPos, &sectionYPos, &sectionWidth, &sectionHeight))
	{
		...
		if (cg_SerenityJediEngineHudMode.integer == 1)
		{
			...
			CG_DrawPic(
				otherHUDBits[OHB_FRAME_RIGHT_MD].xPos,
				otherHUDBits[OHB_FRAME_RIGHT_MD].yPos,
				otherHUDBits[OHB_FRAME_RIGHT_MD].width,
		...
		if (cg_SerenityJediEngineHudMode.integer == 1)
		{
			if (cent->currentState.weapon == WP_SABER)
			{
				CG_DrawForcePower_md(cent, sectionXPos, sectionYPos);
				CG_DrawSaberStyle_md(cent, sectionXPos, sectionYPos);
				CG_DrawblockingMode(cent, sectionXPos, sectionYPos);
				CG_Drawsaberfatigue(cent, sectionXPos, sectionYPos);
			...
			{
				CG_DrawForcePower_md(cent, sectionXPos, sectionYPos);
				CG_DrawAmmo_md(cent, sectionXPos, sectionYPos);
				CG_Drawgunfatigue(cent, sectionXPos, sectionYPos);
			}
			else if (cent->currentState.weapon == WP_MELEE)
			{
				CG_DrawForcePower_md(cent, sectionXPos, sectionYPos);
...
================
*/
static void CG_DrawDatapadForcePower(centity_t* cent, int x, int y)
{
	int			i;
	vec4_t		calcColor;
	float		value, extra = 0, inc, percent;
...
================
*/
static void CG_DrawDataPadAmmo(centity_t* cent, int x, int y)
{
	playerState_t* ps;
	int			numColor_i;
	int			i;
...
void CG_DrawBlockPointBar(centity_t* cent, float chX, float chY, float chW, float chH)
{
	...
	blockPercent = ((float)cent->gent->client->ps.blockPoints / (float)BLOCK_POINTS_MAX);
...
void CG_DrawForcePointBar(centity_t* cent, float chX, float chY, float chW, float chH)
{
	vec4_t aColor;
	vec4_t cColor;
	float x = chX - (chW / 2);
	float y = chY - chH;
	float	ForcePercent = 0.0f;
	...
	ForcePercent = ((float)cent->gent->client->ps.forcePower / (float)cent->gent->client->ps.forcePowerMax);
	...
	//now draw the part to show how much health there is in the color specified
	CG_FillRect(x + 1.0f, y + 1.0f, (ForcePercent * chW) - 1.0f, chH - 1.0f, aColor);

	//then draw the other part greyed out
	CG_FillRect(x + (ForcePercent * chW), y + 1.0f, chW - (ForcePercent * chW) - 1.0f, chH - 1.0f, cColor);
}
...
#define BLOCKPOINT_BAR_RANGE    125
...
void CG_DrawBlockPointBars(void)
{
	...
	{
		cent = &cg_entities[cg_BlockPointBarEnts[i]];
		if (cent && cent->gent)
		{
...
void CG_DrawForcePointBars(void)
{
	float chX = 0, chY = 0;
	centity_t* cent;
	vec3_t pos;
	for (int i = 0; i < cg_numBlockPointBarEnts; i++)
	{
		...
		if (cent && cent->gent)
		{
			...
			if (CG_WorldCoordToScreenCoordFloat(pos, &chX, &chY))
			{//on screen
				CG_DrawForcePointBar(cent, chX, chY, BLOCKPOINT_BAR_WIDTH, BLOCKPOINT_BAR_HEIGHT);
			}
		}
	}
}
...
void CG_AddBlockPointBarEnt(int entNum)
{
	...
	if (DistanceSquared(cg_entities[entNum].lerpOrigin, g_entities[0].client->renderInfo.eyePoint) < (BLOCKPOINT_BAR_RANGE * BLOCKPOINT_BAR_RANGE))
	{
...
#define RADAR_X					(600 - RADAR_RADIUS)
...
float CG_DrawRadar(float y)
{
	...
	for (i = cg.radarEntityCount - 1; i >= 0; i--)
	{
		...
		case ET_PLAYER:
		{
			...
			case TEAM_PLAYER:
				...
				break;
			case TEAM_SOLO:
				VectorCopy(colorTable[CT_MAGENTA], color);
				break;
			case TEAM_FREE:
				VectorCopy(colorTable[CT_CYAN], color);
				break;
			default:
				VectorCopy(colorTable[CT_WHITE], color);
				break;
			}
	...
	cgi_R_SetColor(colorTable[CT_WHITE]);
	CG_DrawRotatePic2(RADAR_X + RADAR_RADIUS + xOffset, y + RADAR_RADIUS, arrow_w, arrow_h, 0, cgs.media.mAutomapPlayerIcon);
...
static void CG_Draw2D(void)
{
	...
	{//force sight is on
		...
		/*if (cg_debugHealthBars.integer == 0)
		{
			CG_DrawHealthBars();
		}*/
	}

	//if (cg.predicted_player_state.communicatingflags & (1 << RESPECTING))
	//{//blockingflag is on test
	//	CG_DrawPic(0, 0, 640, 480, cgi_R_RegisterShader("gfx/2d/droid_view"));
	//}

	//if (cg.predicted_player_state.communicatingflags & (1 << GESTURING))
	//{//blockingflag is on test
	//	CG_DrawPic(0, 0, 640, 480, cgi_R_RegisterShader("gfx/2d/jsense"));
	//}
	...
	if (cg_SerenityJediEngineMode.integer)
	{
		if (cg.predicted_player_state.PlayerEffectFlags & (1 << PEF_BURNING))
		{ //draw it as long as it isn't full
