...
static void CG_CalcIdealThirdPersonViewLocation(void)
{
	...
	if ((!DoingDashAction) && cg.renderingThirdPerson && (cg.snap->ps.forcePowersActive & (1 << FP_SPEED)) && player->client->ps.forcePowerDuration[FP_SPEED])
	{
...
qboolean CG_CalcFOVFromX(float fov_x)
{
	...
	{
		float perc = (float)(cg.stunnedTime - cg.time) / 2500.0f; // goes for 2.5 seconds
...
static qboolean	CG_CalcFov(void) 
{
	...
	else if ((!DoingDashAction) && (!cg.zoomMode || cg.zoomMode > 2) && (cg.snap->ps.forcePowersActive & (1 << FP_SPEED)) && player->client->ps.forcePowerDuration[FP_SPEED])//cg.renderingThirdPerson &&
	{
