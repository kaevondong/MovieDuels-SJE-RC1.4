...
static void PlayCinematic(const char *arg, const char *s, qboolean qbInGame)
{
	...
	if (S_FileExists( arg ))
	{
		...
		if (!Q_stricmp(arg,"video/jk0101_sw.roq"))
		{
			...
			{
				hCrawl = re.RegisterShaderNoMip(va("menu/video/tc_%s", se_language->string));//academy version of text crawl
			}
		...
		else if (!Q_stricmp(arg, "video/openinglogos.roq"))
		{
			psAudioFile = "music/mainMenu/main/OpeningLogosMusic";
			bits |= CIN_system;
		}
		else if (bIsForeign)
		{
