...
//	if (!qbFromSavedGame)
	if (eSavedGameJustLoaded != eFULL)
	{
		...
		{	//not loading full, and directconnect
			...
			client->friendlyfaction = FACTION_NEUTRAL;
...
qboolean ClientSpawn(gentity_t *ent, SavedGameJustLoaded_e eSavedGameJustLoaded )
{
	...
	else
	{
		...
		spawnPoint = SelectSpawnPoint ( ent->client->ps.origin,	(team_t) ent->client->ps.persistant[PERS_TEAM], spawn_origin, spawn_angles);
