...
extern int WP_SaberMustDisruptorBlock(gentity_t* self, gentity_t* atk, qboolean checkBBoxBlock, vec3_t point, int rSaberNum, int rBladeNum);
extern qboolean WP_SaberBlockBolt_MD(gentity_t* self, vec3_t hitloc, qboolean missileBlock);
extern void G_MissileReflectEffect(gentity_t* ent, vec3_t org, vec3_t dir);
extern void WP_ForcePowerDrain(gentity_t* self, forcePowers_t forcePower, int overrideAmt);
extern int WP_SaberBlockCost(gentity_t* defender, gentity_t* attacker, vec3_t hitLocs);
...
//---------------------------------------------------------
{
	...
	{//need to loop this in case we hit a Jedi who dodges the shot
		...
		if (traceEnt)
		{
			if (WP_SaberMustDisruptorBlock(traceEnt, ent, qfalse, tr.endpos, -1, -1) && !WP_DoingForcedAnimationForForcePowers(traceEnt))
			{
				G_MissileReflectEffect(traceEnt, tr.endpos, tr.plane.normal);
			...
			else if (Jedi_DisruptorDodgeEvasion( traceEnt, ent, &tr, HL_NONE ) )
	...
	for ( int i = 0; i < traces; i++ )
	{
		...
		if (traceEnt)
		{
			if (WP_SaberMustDisruptorBlock(traceEnt, ent, qfalse, tr.endpos, -1, -1) && !WP_DoingForcedAnimationForForcePowers(traceEnt))
			{
				...
				WP_ForcePowerDrain(traceEnt, FP_SABER_DEFENSE, WP_SaberBlockCost(traceEnt, ent, tr.endpos));
				//force player into a projective block move.
				hitDodged = WP_SaberBlockBolt_MD(traceEnt, tr.endpos, qtrue);
			}
			else
			{
