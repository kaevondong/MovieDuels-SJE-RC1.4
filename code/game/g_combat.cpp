...
extern qboolean BG_SaberInNonIdleDamageMove(playerState_t* ps);
...
qboolean NPC_IsNotDismemberable(gentity_t* self);
...
void AddFatigueKillBonus(gentity_t* attacker, gentity_t* victim, int meansOfDeath);
void AddFatigueHurtBonus(gentity_t* attacker, gentity_t* victim, int mod);
void AddFatigueHurtBonusMax(gentity_t* attacker, gentity_t* victim, int mod);
...
extern void WP_BlockPointsRegenerate(gentity_t* self, int overrideAmt);
...
void G_StartStasisEffect_FORCE_LEVEL_1(gentity_t* ent, int meFlags = 0, int length = 1000, float timeScale = 0.0f, int spinTime = 0)
{
	if (g_timescale->value != 1.0 || in_camera)
	{//already in some slow-mo mode or in_camera
		return;
...
void G_StartStasisEffect_FORCE_LEVEL_2(gentity_t* ent, int meFlags = 0, int length = 1000, float timeScale = 0.0f, int spinTime = 0)
{
	if (g_timescale->value != 1.0 || in_camera)
	{//already in some slow-mo mode or in_camera
		return;
...
qboolean G_DoDismemberment2(gentity_t* self, vec3_t point, int mod, int damage, int hitLoc, qboolean force = qfalse)
{
	if (mod == MOD_BOWCASTER
		|| (mod == MOD_BLASTER)
		|| (mod == MOD_BLASTER_ALT)
		...
		|| (mod == MOD_BODYSHOT))
	{
		if ((g_gundismemberment->integer > 0))
		{
			if (G_StandardHumanoid(self) && !NPC_IsNotDismemberable(self)
				&& (g_dismemberProbabilities->value > 0.0f || G_Dismemberable2(self, hitLoc)))
			{
				const char* limbBone = NULL, * rotateBone = NULL, * limbTagName = NULL, * stubTagName = NULL;
				int		anim = -1;
				float	limbRollBase = 0, limbPitchBase = 0;
				...
				if (doDismemberment)
				{
					return G_Dismember(self, point, limbBone, rotateBone, limbName, limbCapName, stubCapName, limbTagName, stubTagName, anim, limbRollBase, limbPitchBase, damage, hitLoc);
				}
			}
		}
	}
	...
	{//do dismemberment/twitching
		...
		else
		{
			...
			if (dflags & DAMAGE_DISMEMBER || (meansOfDeath == MOD_SABER))
			{
				...
				|| (meansOfDeath == MOD_REBELBLASTER)
				|| (meansOfDeath == MOD_REBELRIFLE)
				|| (meansOfDeath == MOD_JANGO)
				|| (meansOfDeath == MOD_BOBA)
			...
			{
				GibEntity(self, attacker);
				G_DoDismemberment2(self, self->pos1, meansOfDeath, damage, hitLoc);
			}
			if ( anim >= 0 )
			{
...
extern void RunEmplacedWeapon( gentity_t *ent, usercmd_t **ucmd );
	...
	{// normal death
		...
		if ( anim == -1 )
		{
			...
			else if ((meansOfDeath == MOD_CRUSH && self->s.eFlags & EF_FORCE_GRIPPED))
			{//choked to death
...
void G_Slapdown(gentity_t* self, gentity_t* attacker, const vec3_t pushDir, float strength, qboolean breakSaberLock)
{
	...
	else if (PM_FaceProtectAnim(self->client->ps.torsoAnim))
...
============
*/
int G_LocationDamage(const vec3_t point, gentity_t* targ, gentity_t* attacker, int take)
{
	vec3_t bulletPath;
	vec3_t bulletAngle;
	...
	clientFeetZ = targ->currentOrigin[2] + targ->mins[2];
	// How tall the client is [Relative Z]
	clientHeight = targ->maxs[2] - targ->mins[2];
...
//	}
	...
	if ( mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT )
	{
		if ( client )
		{
			if (client->NPC_class == CLASS_BOBAFETT || client->NPC_class == CLASS_MANDALORIAN || client->NPC_class == CLASS_JANGO)
...
//					CG_DrawEdge(testStartPos,	testEndPos,	EDGE_IMPACT_POSSIBLE);
	...
	if (targ->client && attacker->client && targ->health > 0 && G_StandardHumanoid(targ) && !NPC_IsNotDismemberable(targ))
	{ //do head shots
	...
	if (shieldAbsorbed)
	{
		...
		evEnt = G_TempEntity(targ->currentOrigin, EV_SHIELD_HIT);
	...
	if ( take || (dflags&DAMAGE_NO_DAMAGE) )
	{
		...
		{//two clients
			...
			{//on opposite team
				if (targ->client && targ->health > 0 && attacker && (attacker->client && mod != MOD_FORCE_LIGHTNING) && (attacker->s.number < MAX_CLIENTS || G_ControlledByPlayer(attacker) && attacker->client && mod != MOD_MELEE))
				{//targ is creature and attacker is creature
					if (take > targ->health)
					{//damage is greated than target's health, only give BP/FP reward for damage used to kill victim
...
{//get a bonus for killing an enemy
	...
	if (meansOfDeath == MOD_CRUSH || meansOfDeath == MOD_FORCE_GRIP || meansOfDeath == MOD_FORCE_LIGHTNING || meansOfDeath == MOD_FORCE_DRAIN)
	{
		return;
	}
	...
	if (Manual_Saberblocking(attacker) && (attacker->s.number < MAX_CLIENTS || G_ControlledByPlayer(attacker)))//DONT GET THIS BONUS IF YOUR A BLOCK SPAMMER
	{//add bonus	
		if (g_SerenityJediEngineMode->integer == 2)
		{
	...
	{//add bonus	
		...
		if (attacker->client->ps.saberAttackChainCount >= MISHAPLEVEL_TEN)
		{
...
{//get a small bonus for killing an enemy
	...
	if (mod == MOD_CRUSH || mod == MOD_FORCE_GRIP || mod == MOD_FORCE_LIGHTNING || mod == MOD_FORCE_DRAIN)
	{
		return;
	}
	...
	{//add bonus	
		if (g_SerenityJediEngineMode->integer == 2)
		{
			WP_BlockPointsRegenerate(attacker, FATIGUE_HURTBONUSMIN);
		}
		else
		{
			WP_ForcePowerRegenerate(attacker, FATIGUE_HURTBONUSMIN);
		}
	}
	else
	{//add bonus	
...
void AddFatigueMeleeBonus(gentity_t * attacker, gentity_t * victim)
{//get a small bonus for slapping an enemy
	if (!attacker || !attacker->client || !victim || !victim->client)
	{
		return;
	}
	...
	if (g_SerenityJediEngineMode->integer == 2)
	{
		if (attacker->s.clientNum >= MAX_CLIENTS && !G_ControlledByPlayer(attacker))
		{
			WP_BlockPointsRegenerate(attacker, FATIGUE_NPCDAMAGEBONUS);
		}
		else
		{
...
void AddNPCBlockPointBonus(gentity_t* self)
{//get a small bonus
