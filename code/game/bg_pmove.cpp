...
extern saberMoveName_t PM_KnockawayForParry_MD(int move);
extern saberMoveName_t PM_KnockawayForParry_PB(int move);
extern void G_StartStasisEffect_FORCE_LEVEL_1(gentity_t* ent, int meFlags = 0, int length = 1000, float timeScale = 0.0f, int spinTime = 0);
	...
	else if ( pm->waterlevel < 3 )//|| (pm->ps->waterHeightLevel==WHL_SHOULDERS&&pm->cmd.upmove>0) )
	{
		...
		{//can't do this in zero-G
			...
			{//start a force long-jump!
				...
				if ((g_SerenityJediEngineMode->integer == 2) && pm->gent->client->ps.forcePowerLevel[FP_SPEED] < FORCE_LEVEL_3)
				{//short burst
...
void G_StartDash(gentity_t* ent, int anim)
{
...
qboolean PM_TryRoll(void)
{
	...
	if (g_SerenityJediEngineMode->integer && npc->npc_roll_start)
	{
...
qboolean PM_InSlapDown(playerState_t* ps)
{
	switch (ps->legsAnim)
	{
	case BOTH_KNOCKDOWN1:
...
qboolean PM_BoltBlockingAnim(int anim)
{
	switch (anim)
	{
	case BOTH_P7_S1_B_:
...
qboolean PM_CheckRollGetup(void)
{
	...
	{//lying on back or front
		if (((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())
			&& (pm->cmd.rightmove || (pm->cmd.forwardmove && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0)))//player pressing left or right
...
qboolean PM_GettingUpFromKnockDown(float standheight, float crouchheight)
{
	...
	{//in a knockdown
		...
		if (!pm->ps->legsAnimTimer 
			...
			&& (pm->cmd.upmove > 0 || (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ALORA))))
		{
			if (pm->cmd.upmove < 0)
			{
			...
			else
			{
				...
				{//stand up
					...
					{
					case BOTH_KNOCKDOWN1:
						if ((pm->ps->clientNum && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0) || ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer() && !(pm->ps->userInt3 & (1 << FLAG_FATIGUED)))
							&& pm->cmd.upmove > 0
							&& pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0))
						{
...
qboolean PM_LungeAnim(int anim)
{
	switch (anim)
	{
	case BOTH_LUNGE2_B__T_:	
...
qboolean PM_Saberinstab(int move)
{
	switch (move)
	{
	case LS_ROLL_STAB:
...
qboolean PM_EvasionHoldAnim(int anim)
{
	switch (anim)
	{
...
qboolean PM_EvasionAnim(int anim)
{
	switch (anim)
	{
...
qboolean PM_BlockHoldDualAnim(int anim)
{
	switch (anim)
	{
	case BOTH_BLOCK_HOLD_L_DUAL:
...
qboolean PM_BlockDualAnim(int anim)
{
	switch (anim)
	{
	case BOTH_BLOCK_L_DUAL:
...
qboolean PM_BlockHoldStaffAnim(int anim)
{
	switch (anim)
	{
	case BOTH_BLOCK_HOLD_L_STAFF:
...
qboolean PM_BlockStaffAnim(int anim)
{
	switch (anim)
	{
	case BOTH_BLOCK_L_STAFF:
...
qboolean PM_WalkingOrStanding(gentity_t* self)
...
void PM_JetPackAnim(void)
{
	...
	if (pm->cmd.rightmove > 0)
	{
		...
		{
			PM_SetAnim(pm, SETANIM_TORSO, BOTH_FORCELIGHTNING, SETANIM_AFLAG_PACE);
		}
		else
		{
			anim = BOTH_FORCEJUMPRIGHT1;
...
#define OVERRIDE_ROLL_CHECK g_entities[pm->ps->clientNum].npc_roll_start
	...
	if (pm->ps->pm_flags & PMF_DUCKED || OVERRIDE_ROLL_CHECK)
	{
			...
			&& (!PM_InRollIgnoreTimer(pm->ps) || (!pm->ps->legsAnimTimer && pm->cmd.upmove < 0)))//not in a roll (or you just finished one and you're still holding crouch)
		{
			...
			if ((PM_CrouchAnim(pm->gent->client->ps.legsAnim)) && (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
				(g_SerenityJediEngineMode->integer &&
				(pm->cmd.buttons & BUTTON_USE && pm->ps->pm_flags & PMF_USE_HELD) || (OVERRIDE_ROLL_CHECK)))
			{//roll!
				rolled = PM_TryRoll_MD();
			}
...
static void PM_BeginWeaponChange( int weapon )
{
	...
	if (pm->ps->weapon == WP_SABER && pm->ps->ManualBlockingFlags & (1 << MBF_BLOCKING) || pm->ps->ManualBlockingFlags & (1 << MBF_PROJBLOCKING))
	{
		return;
	}
...
{
	int anim = BOTH_SABERSINGLECROUCH;
...
int PM_ReadyPoseForSaberAnimLevelMD(void)
{
	int anim = BOTH_STAND2;

	qboolean HoldingBlock = (pm->ps->ManualBlockingFlags & (1 << MBF_BLOCKING) ? qtrue : qfalse); //Holding Block Button
	...
	if (pm->ps->forcePower <= BLOCKPOINTS_FAIL || pm->ps->blockPoints <= BLOCKPOINTS_FAIL)
	{
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				{
				case SS_DUAL:
					if (PM_StandingidleAnim(pm->ps->legsAnim) && HoldingBlock)
					{
						anim = BOTH_SABERDUAL_STANCE;
					}
					else if (PM_WalkingOrRunningAnim(pm->ps->legsAnim))
					{
						anim = BOTH_SABERDUAL_STANCE;
					}
					else
				...
				case SS_STAFF:
					...
					{
						anim = BOTH_SABERSTAFF_WALK_STANCE;
					}
					else
					{
						anim = BOTH_STAND1;
...
{//Sets your saber block position based on your current movement commands.
	int anim = PM_ReadyPoseForSaberAnimLevelAMD();

	qboolean HoldingBlock = (pm->ps->ManualBlockingFlags & (1 << MBF_BLOCKING) ? qtrue : qfalse); //Normal Blocking

	signed char forwardmove = pm->cmd.forwardmove;
	signed char rightmove = pm->cmd.rightmove;

	if (pm->ps->forcePower < BLOCKPOINTS_DANGER)
	{
	...
	if ((pm->ps->eFlags & EF_FORCE_DRAINED) || (pm->ps->eFlags & EF_FORCE_GRIPPED))
	{
		return -1;
	}
	...
	else
	{
		...
		else if ((HoldingBlock) && pm->cmd.upmove <= 0)
		{
			if (forwardmove < 0)
			{//walking backwards
				if (rightmove < 0)
...
int PM_BlockingPoseForSaberAnimLevelDualAMD(void)
{//Sets your saber block position based on your current movement commands.
	int anim = PM_ReadyPoseForSaberAnimLevelAMD();
...
{//Sets your saber block position based on your current movement commands.
	...
	qboolean ActiveBlocking = (pm->ps->ManualBlockingFlags & (1 << MBF_PROJBLOCKING) ? qtrue : qfalse); //Active Blocking
	...
	if (pm->ps->forcePower < BLOCKPOINTS_DANGER)
	{
		return PM_ReadyPoseForSaberAnimLevelAMD();
	}
	...
	else
	{
		...
		else if ((ActiveBlocking) && pm->cmd.upmove <= 0)
		{
			if (forwardmove < 0)
			{//walking backwards
				if (rightmove < 0)
...
void PM_AddBoltBlockFatigue(playerState_t* ps, int Fatigue)
{
	if (g_SerenityJediEngineMode->integer == 2)
	{
		...
		if (g_SerenityJediEngineMode->integer == 2)
		{
			if (ps->blockPoints < BLOCKPOINTS_TEN)
			{
...
qboolean PM_IsInBlockingAnim(int move)
{
	switch (move)
	{
	case BOTH_P1_S1_T_:
	case BOTH_P1_S1_T__MD:
	case BOTH_P1_S1_BL:
	case BOTH_P1_S1_BR:
	case BOTH_P1_S1_TL:
	case BOTH_P1_S1_TL_MD:
	case BOTH_P1_S1_TR:
	case BOTH_P1_S1_TR_MD:
	case BOTH_P1_S1_B_:

	case BOTH_P6_S6_T_:
	case BOTH_P6_S6_T__MD:
	case BOTH_P6_S6_BL:
	case BOTH_P6_S6_BR:
	case BOTH_P6_S6_TL:
	case BOTH_P6_S6_TL_MD:
	case BOTH_P6_S6_TR:
	case BOTH_P6_S6_TR_MD:
	case BOTH_P6_S1_B_:

	case BOTH_P7_S7_T_:
	case BOTH_P7_S7_T__MD:
	case BOTH_P7_S7_BL:
	case BOTH_P7_S7_BR:
	case BOTH_P7_S7_TL:
	case BOTH_P7_S7_TL_MD:
	case BOTH_P7_S7_TR:
	case BOTH_P7_S7_TR_MD:
	case BOTH_P7_S1_B_:
		return qtrue;
	}
	return qfalse;
...
void PM_NPCFatigue(playerState_t* ps, int newMove, int anim)
{
	if (g_SerenityJediEngineMode->integer == 2)
	{
		...
		{//wasn't playing that attack before
			...
			else if (PM_KnockawayForParry_MD(newMove))
			{//simple saber attack
				PM_AddBlockFatigue(ps, Fatigue_SaberAttack(ps));
			}
			else if (PM_KnockawayForParry_PB(newMove))
			{//simple saber attack
				PM_AddBlockFatigue(ps, Fatigue_SaberAttack(ps));
			}
	...
	else
	{
		...
		{//wasn't playing that attack before
			...
			{//simple saber attack
				PM_AddFatigue(ps, Fatigue_SaberAttack(ps));
			}
			else if (PM_KnockawayForParry(newMove))
			{//simple saber attack
				PM_AddFatigue(ps, Fatigue_SaberAttack(ps));
...
void PM_BlockFatigue(playerState_t* ps, int newMove, int anim)
{
	if (g_SerenityJediEngineMode->integer == 2)
	{
		if (ps->saberMove != newMove)
		{//wasn't playing that attack before
			...
			else if (PM_SaberInAttack(newMove) || PM_SaberInSpecialAttack(anim))
			{//simple saber attack
				PM_AddBlockFatigue(ps, FATIGUE_AUTOSABERDEFENSE);
			}
			else if (PM_KnockawayForParry(newMove))
...
void PM_SaberFatigue(playerState_t* ps, int newMove, int anim)
{
	if (g_SerenityJediEngineMode->integer == 2)
	{
		if (ps->saberMove != newMove)
		{//wasn't playing that attack before
			...
			{//simple saber attack
				PM_AddBlockFatigue(ps, FATIGUE_GROUNDATTACK);
			}
			else if (PM_SaberInAttack(newMove) || PM_SaberInSpecialAttack(anim))
			{//simple saber attack
				PM_AddBlockFatigue(ps, Fatigue_SaberAttack(ps));
...
void PM_SetSaberMove(saberMoveName_t newMove)
{
	...
	{
		Com_Printf("SetSaberMove:  From '%s' to '%s'\n", saberMoveData[pm->ps->saberMove].name, saberMoveData[newMove].name);
	}
	...
	if (g_SerenityJediEngineMode->integer == 2)
	{
		...
		else if (pm->ps->saberAttackChainCount < MISHAPLEVEL_HUDFLASH)
		{
			pm->ps->userInt3 &= ~(1 << FLAG_ATTACKFATIGUE);
		}
	}
	...
	if (newMove == LS_READY)
	{
		if (pm->ps->saberBlockingTime > cg.time)
		{
			...
			{
				if ((ActiveBlocking) && g_SerenityJediEngineMode->integer == 2)
				{
					if (pm->ps->saberAnimLevel == SS_DUAL)
					{
				...
				else if ((HoldingBlock && !ActiveBlocking) && g_SerenityJediEngineMode->integer == 2)
				{
					if (pm->ps->saberAnimLevel == SS_DUAL)
					{
		...
		else
		{
			...
			else
			{
				if (g_SerenityJediEngineMode->integer)
				{
					...
					{
						anim = PM_ReadyPoseForSaberAnimLevelMD();
					}
				}
				else
				{
	...
	else if (newMove == LS_A_JUMP_T__B_ && pm->ps->saberAnimLevel == SS_DESANN && g_SerenityJediEngineMode->integer)
	{
		anim = BOTH_FJSS_TR_BL;
	}
	...
	{//staff has an entirely new set of anims, besides special attacks
		...
		{//there aren't 1-7, just 1, 6 and 7, so just set it
			anim = BOTH_P7_S7_T_ + (anim - BOTH_P1_S1_T_);
		}
		else
	...
	else if (pm->ps->saberAnimLevel == SS_STAFF && newMove >= LS_S_TL2BR && newMove < LS_REFLECT_LL_MD)
	{//staff has an entirely new set of anims, besides special attacks
		if (newMove >= LS_V1_BR && newMove <= LS_REFLECT_LL_MD)
		{//there aren't 1-7, just 1, 6 and 7, so just set it
			anim = BOTH_P7_S7_T__MD + (anim - BOTH_P1_S1_T__MD);
		}
		else
		{//add the appropriate animLevel
			anim += (pm->ps->saberAnimLevel - FORCE_LEVEL_1) * SABER_ANIM_GROUP_SIZE;
	...
	else if ((pm->ps->saberAnimLevel == SS_DUAL || (pm->ps->dualSabers && pm->ps->saber[1].Active()))
		&& newMove >= LS_S_TL2BR && newMove < LS_REFLECT_LL)
	{//staff has an entirely new set of anims, besides special attacks
		if (newMove >= LS_V1_BR && newMove <= LS_REFLECT_LL)
		{//there aren't 1-7, just 1, 6 and 7, so just set it
			anim = BOTH_P6_S6_T_ + (anim - BOTH_P1_S1_T_);
		}
		...
		&& newMove >= LS_S_TL2BR && newMove < LS_REFLECT_LL_MD)
	{//staff has an entirely new set of anims, besides special attacks
		if (newMove >= LS_V1_BR && newMove <= LS_REFLECT_LL_MD)
		{//there aren't 1-7, just 1, 6 and 7, so just set it
			anim = BOTH_P6_S6_T__MD + (anim - BOTH_P1_S1_T__MD);//shift it up to the proper set
		}
		else if ((newMove == LS_A_R2L || newMove == LS_S_R2L || newMove == LS_A_L2R || newMove == LS_S_L2R)
			&& PM_CanDoDualDoubleAttacks()
			&& G_CheckEnemyPresence(pm->gent, DIR_RIGHT, 150.0f)
			&& G_CheckEnemyPresence(pm->gent, DIR_LEFT, 150.0f))
		{//enemy both on left and right
		...
		else if ((newMove == LS_A_T2B || newMove == LS_S_T2B || newMove == LS_A_BACK || newMove == LS_A_BACK_CR)
			&& PM_CanDoDualDoubleAttacks()
			&& G_CheckEnemyPresence(pm->gent, DIR_FRONT, 150.0f)
			&& G_CheckEnemyPresence(pm->gent, DIR_BACK, 150.0f))
		{//enemy both in front and back
	...
	if ((anim == BOTH_STAND1 && (pm->ps->saber[0].type == SABER_ARC || (pm->ps->dualSabers && pm->ps->saber[1].Active())))
		|| anim == BOTH_STAND2
		|| (0 && (anim == BOTH_SABERSTAFF_STANCE || anim == BOTH_SABERSTAFF_WALK_STANCE))
		|| anim == BOTH_SABERDUAL_STANCE
		|| anim == BOTH_SABERFAST_STANCE
		|| anim == BOTH_SABERSLOW_STANCE
		|| anim == BOTH_SABERTAVION_STANCE
	...
	if (!PM_RidingVehicle())
	{
		if (!manualBlocking)
		{
			...
			else if ((!pm->cmd.forwardmove && !pm->cmd.rightmove && !pm->cmd.upmove && !(pm->ps->pm_flags & PMF_DUCKED)) || pm->ps->speed == 0 || ((ActiveBlocking) && g_SerenityJediEngineMode->integer == 2) || ((HoldingBlock && !ActiveBlocking) && g_SerenityJediEngineMode->integer == 2))
			{//not trying to run, duck or jump
				if ((((ActiveBlocking) && g_SerenityJediEngineMode->integer == 2) || ((HoldingBlock && !ActiveBlocking) && g_SerenityJediEngineMode->integer == 2))
					&& !PM_SaberInParry(newMove)
					&& !PM_SaberInKnockaway(newMove)
					&& !PM_SaberInBrokenParry(newMove)
					&& !PM_SaberInReflect(newMove)
	...
	{//successfully changed anims //special check for *starting* a saber swing

		if (pm->ps->weapon == WP_SABER && pm->ps->SaberLength() > 1)
		{
			if (g_SerenityJediEngineMode->integer)
			{
				if (g_SerenityJediEngineMode->integer == 2)
				{
					if (pm->gent && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent)))
					{
						if (HoldingBlock)
						{
							if (ActiveBlocking)
							{
								PM_BlockFatigue(pm->ps, newMove, anim);//drainblockpoints low cost
							}
							else
							{
								PM_SaberFatigue(pm->ps, newMove, anim);//drainblockpoints highcost
							}
						}
					}
					else
					{
						PM_NPCFatigue(pm->ps, newMove, anim);//drainblockpoints low cost
					}
				}
				else
				{
			...
			if (!PM_SaberInBounce(newMove) && !PM_SaberInReturn(newMove))
		...
		else if (pm->ps->saberBlocked <= BLOCKED_ATK_BOUNCE || !pm->ps->SaberActive() || (newMove < LS_PARRY_UR_MD || newMove > LS_REFLECT_LL_MD))
		{//NPCs only clear blocked if not blocking?
			pm->ps->saberBlocked = BLOCKED_NONE;
		}
...
saberMoveName_t PM_NPCSaberAttackFromQuad( int quad )
{
	...
	if ( pm->gent && ((pm->gent->NPC && pm->gent->NPC->rank != RANK_ENSIGN && pm->gent->NPC->rank != RANK_CIVILIAN ) || (pm->gent->client && (pm->gent->client->NPC_class == CLASS_TAVION || pm->gent->client->NPC_class == CLASS_GALEN ||pm->gent->client->NPC_class == CLASS_ALORA))) )
	{
				...
				{//JK2-style
					...
					if (winAnim != BOTH_CCWCIRCLEBREAK)
					{
						if ((!genemy->s.number && genemy->health <= 25)//player low on health
							|| (genemy->s.number && genemy->client->NPC_class != CLASS_KYLE && genemy->client->NPC_class != CLASS_LUKE && genemy->client->NPC_class != CLASS_TAVION && genemy->client->NPC_class != CLASS_ALORA && genemy->client->NPC_class != CLASS_VADER && genemy->client->NPC_class != CLASS_DESANN)//any NPC that's not a boss character
...
int G_SaberLockStrength(gentity_t* gent)
{
	...
	if (gent->s.number >= MAX_CLIENTS)
	{
		...
		else if (gent->client->NPC_class == CLASS_VADER || gent->client->NPC_class == CLASS_LUKE)
		{
			strength += 3 + Q_irand(0, g_spskill->integer);
		}
		else
		{
...
saberMoveName_t G_PickAutoKick( gentity_t *self, gentity_t *enemy, qboolean storeMove )
{
	...
	{//generally to one side
		if ( rDot > 0 )
		{//kick right
			if (g_SerenityJediEngineMode->integer == 2 && self->client->ps.weapon == WP_SABER && self->client->ps.SaberActive() && G_CheckEnemyPresence(self, DIR_RIGHT, 64, 12.0f))
			{
				kickMove = LS_SLAP_R;
			}
			else
		...
		{//kick left
			if (g_SerenityJediEngineMode->integer == 2 && self->client->ps.weapon == WP_SABER && self->client->ps.SaberActive() && G_CheckEnemyPresence(self, DIR_LEFT, 64, 12.0f))
			{
				kickMove = LS_SLAP_L;
			}
			else
	...
	{//generally in front or behind us
		if ( fDot > 0 )
		{//kick fwd
			if (g_SerenityJediEngineMode->integer == 2 && self->client->ps.weapon == WP_SABER && self->client->ps.SaberActive() && G_CheckEnemyPresence(self, DIR_FRONT, 64, 12.0f))
			{
...
qboolean PM_CanDoKata( void )
{
	...
	if (g_SerenityJediEngineMode->integer )
	{
		if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) //npc kata restrictions
			&& (pm->ps->blockPoints <= BLOCKPOINTS_MISSILE || pm->ps->forcePower <= BLOCKPOINTS_MISSILE))
		{
			return qfalse;
		}
	}
	...
	if ( !pm->ps->saberInFlight//not throwing saber
		...
		&& pm->ps->saberAttackChainCount < MISHAPLEVEL_TEN
	...
	{//player kicks
		...
		{//kick to side
			...
			{//kick right
				...
				{
					if (g_SerenityJediEngineMode->integer == 2 && pm->ps->weapon == WP_SABER && pm->ps->SaberActive() && G_CheckEnemyPresence(pm->gent, DIR_RIGHT, 64, 12.0f))
					{
			...
			{//kick left
				...
				{
					if (g_SerenityJediEngineMode->integer == 2 && pm->ps->weapon == WP_SABER && pm->ps->SaberActive() && G_CheckEnemyPresence(pm->gent, DIR_LEFT, 64, 12.0f))
					{
		...
		{//kick front/back
			...
			{//kick fwd
				...
				else if (g_SerenityJediEngineMode->integer > 0 && pm->ps->groundEntityNum != ENTITYNUM_NONE && pm->ps->weapon == WP_SABER && pm->ps->SaberActive() && G_CheckEnemyPresence(pm->gent, DIR_FRONT, 64, 12.0f))
				{
					if (pm->ps->blockPoints < BLOCKPOINTS_HALF || pm->ps->forcePower < BLOCKPOINTS_HALF)
					{
		...
		{//auto-pick?
			...
			{//kicked!
				...
				{//just pull back
					...
					{
						if (d_slowmoaction->integer && (pm->gent->owner->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent->owner)))
						{
...
void PM_MeleeMoveForConditions(void)
{
	if (pm->gent && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent))) //player only restrictions
	{
		...
		if (pm->ps->forcePower < BLOCKPOINTS_FAIL)
		{
			return;
		}
	}

	if (!PM_KickMove(pm->ps->saberMove))//not trying to duck
	{//player kicks
		if (pm->cmd.rightmove)
		{//kick to side
			if (pm->cmd.rightmove > 0)
			{//kick right
				...
				{
					if ((g_SerenityJediEngineMode->integer == 2) && pm->ps->weapon == WP_SABER && pm->ps->SaberActive())
					{
		...
		{//kick front/back
			...
			{//kick fwd
				...
				else if (pm->ps->groundEntityNum != ENTITYNUM_NONE && pm->ps->weapon == WP_SABER && pm->ps->SaberActive()&& G_CheckEnemyPresence(pm->gent, DIR_FRONT, 64, 12.0f))
				{//don't jump while doing this move and don't do this move if in air
					if (g_SerenityJediEngineMode->integer)
					{
						if (g_SerenityJediEngineMode->integer == 2)
						{
...
int PM_SetStandingSaberBlockDual(playerState_t* ps, usercmd_t* cmd, gentity_t* gent)
{
	int anim = -1;
	...
	if (pm->ps->weapon == WP_SABER
		&& pm->ps->SaberActive()
		&& !pm->ps->saberInFlight
		&& !PM_KickMove(pm->ps->saberMove)//not already in a kick
		&& (pm->cmd.forwardmove >= 0)
		&& !(pm->ps->pm_flags & PMF_DUCKED)
		&& pm->ps->blockPoints > BLOCKPOINTS_DANGER
		&& pm->ps->forcePower > BLOCKPOINTS_DANGER)
	{
		if (G_OkayToLean(ps, cmd, qtrue))  //pushing a direction
		{
			if (pm->cmd.rightmove > 0)
			{//lean right
				if (pm->ps->torsoAnim == BOTH_BLOCK_HOLD_R_DUAL)
				{
			...
			{//lean left
				if (pm->ps->torsoAnim == BOTH_BLOCK_HOLD_L_DUAL)
				{
					anim = pm->ps->torsoAnim;
				}
				else
			...
			else if (!pm->cmd.forwardmove && !pm->cmd.rightmove && (pm->cmd.buttons & BUTTON_ATTACK))
			{
				pm->ps->saberBlocked = BLOCKED_TOP_MD;
			}
		}
		else if (!pm->cmd.forwardmove && !pm->cmd.rightmove && (pm->cmd.buttons & BUTTON_ATTACK))
		{
		...
		{
			int extraHoldTime = 0;
			if (PM_BlockDualAnim(pm->ps->torsoAnim) && !PM_BlockHoldDualAnim(pm->ps->torsoAnim))
			...
			else
			{
				...
				if (g_SerenityJediEngineMode->integer == 2)
				{
					PM_AddBlockFatigue(pm->ps, FATIGUE_SABERBLOCK);
				}
				else
				{
					PM_AddFatigue(pm->ps, FATIGUE_SABERBLOCK);
				}
			}
...
int PM_SetStandingSaberBlockStaff(playerState_t* ps, usercmd_t* cmd, gentity_t* gent)
{
	int anim = -1;
		...
		&& pm->ps->forcePower > BLOCKPOINTS_DANGER)
	{
		if (G_OkayToLean(ps, cmd, qtrue))  //pushing a direction
		{
			if (pm->cmd.rightmove > 0)
			{//lean right
				if (pm->ps->torsoAnim == BOTH_BLOCK_HOLD_R_STAFF)
				{
					anim = pm->ps->torsoAnim;
				}
				else
			...
			{//lean left
				if (pm->ps->torsoAnim == BOTH_BLOCK_HOLD_L_STAFF)
				{
					anim = pm->ps->torsoAnim;
				}
				else
		...
		{
			int extraHoldTime = 0;
			if (PM_BlockStaffAnim(pm->ps->torsoAnim) && !PM_BlockHoldStaffAnim(pm->ps->torsoAnim))
			{//use the hold pose, don't start it all over again
...
int PM_SetStandingSaberBlock(playerState_t* ps, usercmd_t* cmd, gentity_t* gent)
{
	int anim = -1;
			...
			if (pm->ps->saberBlocked)
			{
				...
				case BLOCKED_ATK_BOUNCE:
					...
					{//start the bounce move
						...
						PM_SetSaberMove((saberMoveName_t)bounceMove);
						pm->ps->weaponTime = pm->ps->torsoAnimTimer;
					}
					break;
				...
				case BLOCKED_UPPER_RIGHT_MD:
					...
					{
						PM_SetSaberMove(LS_PARRY_UR_MD);
					}
				...
				case BLOCKED_UPPER_RIGHT_PROJ:
					if (g_SerenityJediEngineMode->integer)
					{
						PM_SetSaberMove(LS_REFLECT_UR_MD);
					}
					else
					{
				...
				case BLOCKED_UPPER_LEFT_MD:
					...
					{
						PM_SetSaberMove(LS_PARRY_UL_MD);
					}
				...
				case BLOCKED_UPPER_LEFT_PROJ:
					...
					{
						PM_SetSaberMove(LS_REFLECT_UL_MD);
					}
					wasAttackedByGun = qtrue;
				...
				case BLOCKED_LOWER_RIGHT_MD:
					...
					{
						PM_SetSaberMove(LS_PARRY_LR_MD);
					}
				...
				case BLOCKED_LOWER_RIGHT_PROJ:
					if (g_SerenityJediEngineMode->integer)
					{
						PM_SetSaberMove(LS_REFLECT_LR_MD);
					}
					else
					{
				...
				case BLOCKED_LOWER_LEFT_MD:
					...
					{
						PM_SetSaberMove(LS_PARRY_LL_MD);
					}
				...
				case BLOCKED_LOWER_LEFT_PROJ:
					if (g_SerenityJediEngineMode->integer)
					{
						PM_SetSaberMove(LS_REFLECT_LL_MD);
					}
					else
					{
				...
				case BLOCKED_TOP_MD:
					...
					{
						PM_SetSaberMove(LS_PARRY_UP_MD);
					}
				...
				case BLOCKED_TOP_PROJ:
					if (g_SerenityJediEngineMode->integer)
					{
						PM_SetSaberMove(LS_REFLECT_UP_MD);
					}
					else
					{
...
{
	int anim = PM_ReadyPoseForSaberAnimLevelMD();
	...
	if (HoldingBlock && pm->cmd.upmove <= 0
		&& (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()))
	{
		...
		{//standing still block
			...
			else if (!pm->cmd.forwardmove && !pm->cmd.rightmove && (ActiveBlocking))
			{
				PM_SetSaberMove(LS_PARRY_UP_MD);
			}
			else
...
void PM_WeaponLightsaber(void)
{
	...
	if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer() && g_SerenityJediEngineMode->integer)
	...
	if (pm->ps->weaponTime > 0)
	{
		...
		if (pullmove != LS_NONE)
		{
			...
			pm->ps->weaponstate = WEAPON_READY;
			PM_SetSaberMove((saberMoveName_t)pullmove);
			return;
		}
	...
	else
	{
		...
		{//set up the block position
			if (pm->ps->saberAnimLevel == SS_DUAL)
			{
				if (!PM_BoltBlockingAnim(pm->ps->torsoAnim))
				{
					PM_SetStandingSaberBlockDual(pm->ps, &pm->cmd, pm->gent);
				}
			}
			else if (pm->ps->saberAnimLevel == SS_STAFF)
			{
				if (!PM_BoltBlockingAnim(pm->ps->torsoAnim))
				{
					PM_SetStandingSaberBlockStaff(pm->ps, &pm->cmd, pm->gent);
				}
			}
			else
			{
				if (!PM_BoltBlockingAnim(pm->ps->torsoAnim))
				{
					PM_SetStandingSaberBlock(pm->ps, &pm->cmd, pm->gent);
				}
			}
		}
		...
		else
		{
				...
				&&(pm->cmd.buttons&BUTTON_ATTACK) )
			{
				if ( !g_saberNewControlScheme->integer )
				{
					...
					if ( pullAtk != LS_NONE )
					{
						PM_SetSaberMove((saberMoveName_t)pullAtk );
						pm->ps->weaponstate = WEAPON_FIRING;
						return;
					}
				}
		...
		if(pm->gent && (pm->gent->s.number<MAX_CLIENTS||G_ControlledByPlayer(pm->gent)))
		{
			...
			default:
				...
				else if ((HoldingBlock && !ActiveBlocking) && g_SerenityJediEngineMode->integer == 2)
				{
					if (pm->ps->saberAnimLevel == SS_DUAL)
					{
						int anim = PM_BlockingPoseForSaberAnimLevelDualAMD();
					}
					else if (pm->ps->saberAnimLevel == SS_STAFF)
					{
						int anim = PM_BlockingPoseForSaberAnimLevelStaffAMD();
					}
					else
					{
						int anim = PM_BlockingPoseForSaberAnimLevelSingleAMD();
					}
				}
				else
				{
	...
	if ( PM_CanDoKata() )
	{
		...
		{
			PM_SetSaberMove((saberMoveName_t)overrideMove );
			pm->ps->weaponstate = WEAPON_FIRING;
			if ( pm->gent )
			{
	...
	if (!delayed_fire)
	{
		...
		{	// Last attack is not yet complete.
			...
			else if ((HoldingBlock && !ActiveBlocking) && g_SerenityJediEngineMode->integer == 2)
			{
				if (pm->ps->saberAnimLevel == SS_DUAL)
				{
					PM_SetAnim(pm, SETANIM_TORSO, PM_BlockingPoseForSaberAnimLevelDualAMD(), SETANIM_FLAG_OVERRIDE);
					return;
				}
				else if (pm->ps->saberAnimLevel == SS_STAFF)
				{
					PM_SetAnim(pm, SETANIM_TORSO, PM_BlockingPoseForSaberAnimLevelStaffAMD(), SETANIM_FLAG_OVERRIDE);
				}
				else
				{
					PM_SetAnim(pm, SETANIM_TORSO, PM_BlockingPoseForSaberAnimLevelSingleAMD(), SETANIM_FLAG_OVERRIDE);
					return;
				}
			}
			else
		...
		else
		{
			...
			else if (curmove >= LS_PARRY_UP_MD && curmove <= LS_REFLECT_LL_MD)
			{//from a parry or reflection, can go directly into an attack
				if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer())
				{//NPCs
					newmove = PM_NPCSaberAttackFromQuad(saberMoveData[curmove].endQuad);
			...
			if ( anim == -1 )
			{
				...
				{//get attack move from movement command
					...
					else
					{
						...
						if (g_SerenityJediEngineMode->integer == 2 && pm->ps->blockPoints > BLOCKPOINTS_HALF && pm->ps->saberAttackChainCount < MISHAPLEVEL_TEN)
						{
	...
	if (pm->ps->weaponTime > 0 && ActiveBlocking && g_SerenityJediEngineMode->integer == 2)
	{
		if (pm->ps->saberAnimLevel == SS_STAFF)
		{
			PM_SetAnim(pm, SETANIM_TORSO, PM_BlockingPoseForSaberAnimLevelStaff(), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
		}
		else if (pm->ps->saberAnimLevel == SS_DUAL)
		{
			PM_SetAnim(pm, SETANIM_TORSO, PM_BlockingPoseForSaberAnimLevelDual(), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
		}
		else
		{
			PM_SetAnim(pm, SETANIM_TORSO, PM_BlockingPoseForSaberAnimLevelSingle(), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
		}
		PM_SetSaberMove(LS_READY);
	}
	else if (pm->ps->weaponTime > 0 && (HoldingBlock && !ActiveBlocking) && g_SerenityJediEngineMode->integer == 2)
	{
		if (pm->ps->saberAnimLevel == SS_STAFF)
		{
		...
		{//already playing it
			...
			{//anim was done, set it back to ready
				...
				if (pm->cmd.buttons&BUTTON_ATTACK)
				{
					if ( !pm->ps->weaponTime )
					{
	 					PM_SetSaberMove((saberMoveName_t)saberMove );
						pm->ps->weaponstate = WEAPON_FIRING;
						pm->ps->weaponTime = pm->ps->torsoAnimTimer;
					}
				}
...
qboolean BG_SaberInFullDamageMove(playerState_t* ps)
...
qboolean BG_SaberInNonIdleDamageMove(playerState_t* ps)
