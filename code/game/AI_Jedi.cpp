...
extern saberMoveName_t PM_KnockawayForParry_AMD(int move);
...
qboolean NPC_ForceMaster(gentity_t* self)
{
	switch (self->client->NPC_class)
	{
	case CLASS_SITHLORD:
...
qboolean NPC_IsSaberMaster(gentity_t* self)
{
	switch (self->client->NPC_class)
	{
	case CLASS_DESANN:
...
qboolean NPC_IsSithLord(gentity_t* self)
{
	switch (self->client->NPC_class)
	{
	case CLASS_DESANN:
...
qboolean NPC_IsStaffStyle(gentity_t* self)
{
...
qboolean NPC_IsDualStyle(gentity_t* self)
{
...
qboolean Jedi_CultistDarkForceUser(gentity_t* self)
{
	if (!self || !self->client)
	{
		return qfalse;
...
qboolean Jedi_StopKnockdown(gentity_t* self, gentity_t* pusher, const vec3_t pushDir)
{
	...
	{//flip
		...
		{
			self->client->ps.forceJumpCharge = 10;//FIXME: calc this intelligently?
		}
		else
		{
...
static void Jedi_AggressionErosion(int amt)
{
	...
	else if (NPCInfo->stats.aggression < 4 || (NPCInfo->stats.aggression < 6 && NPC_IsDarkJedi(NPC)))
	{//turn off the saber
...
static qboolean Jedi_Track(void)
{
	...
	{//approach enemy
		NPCInfo->combatMove = qtrue;
		NPC_SetMoveGoal(NPC, NPCInfo->enemyLastSeenLocation, 16, qtrue, 0, NPC->enemy);
		if (NPC_MoveToGoal(qfalse))
		{
			NPC_UpdateAngles(qtrue, qtrue);
			return qtrue;
...
static void Jedi_StartRetreat(void)
{
	TIMER_Set(NPC, "roamTime", -level.time);
	TIMER_Set(NPC, "strafeLeft", -level.time);
	TIMER_Set(NPC, "strafeRight", -level.time);
...
static qboolean Jedi_DecideKick(void)
{
	...
	if (NPC->client->ps.weapon == WP_SABER)
	{
		...
		else if (NPC->client->ps.dualSabers && (NPC->client->ps.saber[1].saberFlags & SFL_NO_KICKS))
		{
			return qfalse;
		}
		else if ((NPC->client->ps.saber[0].saberFlags & SFL_NOT_THROWABLE))
		{
			return qfalse;
		}
...
{
	WP_SabersCheckLock2(NPC, NPC->enemy, (sabersLockMode_t)Q_irand(LOCK_KYLE_GRAB1, LOCK_KYLE_GRAB2));//LOCK_KYLE_GRAB3
	TIMER_Set(NPC, "NPC_GrabPlayerDebounce", NPC->client->ps.torsoAnimTimer + Q_irand(120000, 240000));
}
...
qboolean Kyle_CanDoGrab(void)
{
	if (g_SerenityJediEngineMode->integer == 2 && (NPCInfo->aiFlags & NPCAI_BOSS_SERENITYJEDIENGINE))
	{
		if (NPC->client->NPC_class == CLASS_KYLE
			...
			&& (NPC->client->ps.saberAttackChainCount < MISHAPLEVEL_FOUR)
...
extern void AddNPCBlockPointBonus(gentity_t* self);
...
static void Jedi_CombatDistance(int enemy_dist)
{
	...
	if (g_SerenityJediEngineMode->integer == 2 && enemy_dist < 100
		&& NPC->enemy
		&& NPC->enemy->client
		...
		&& (PM_SaberInKata((saberMoveName_t)NPC->enemy->client->ps.saberMove)))
	{//whoa, back off!!!
		if (Q_irand(0, 1))
		{
	...
	if (g_SerenityJediEngineMode->integer == 2 && enemy_dist > 256
	...
	if (TIMER_Done(NPC, "heal") && enemy_dist > 256
		&& !Jedi_SaberBusy(NPC) && NPC_IsAlive(NPC, NPC->enemy)
	...
	if (g_SerenityJediEngineMode->integer && NPC_IsAlive(NPC, NPC->enemy)
		&& enemy_dist <= 150
		&& NPC->enemy
		&& NPC->enemy->client
		&& NPC->client->ps.weapon == WP_SABER
		...
		&& InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.7f))
	{
		if (enemy_dist <= 150 && (NPC->client->ps.saberAnimLevel == SS_DUAL)
			|| (NPC->client->ps.saberAnimLevel == SS_STAFF))
		{
	...
	if (g_SerenityJediEngineMode->integer && NPC_CanDoSlap()
		...
		&& !PM_InKnockDown(&NPC->client->ps))
	{
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.3f))
				{
					...
					G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin, (g_spskill->integer + 1) * Q_irand(2, 5), DAMAGE_NO_KNOCKBACK, MOD_MELEE);
				 
					if (NPC->client->ps.torsoAnim == BOTH_A7_SLAP_L)
					{//smackdown
						int knockAnim = BOTH_SLAPDOWNLEFT;
					...
					else if (NPC->client->ps.torsoAnim == BOTH_A7_SLAP_R)
					{
						G_Throw(NPC->enemy, smackDir, 40);
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_SLAPDOWNRIGHT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else if (NPC->client->ps.torsoAnim == BOTH_SWEEP_KICK)
					{
						G_Throw(NPC->enemy, smackDir, 40);
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_KNOCKDOWN3, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else if (NPC->client->ps.torsoAnim == BOTH_A7_HILT)
					{
						G_Throw(NPC->enemy, smackDir, 40);
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_BASHED1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else
					{//uppercut
					 //throw them
						...
						//make them backflip
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
		...
		else
		{
			if (enemyDist < MELEE_DIST_SQUARED
				&& !NPC->client->ps.weaponTime //not firing
				&& !PM_InKnockDown(&NPC->client->ps)//not knocked down
				&& InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.3f))//within 80 and in front
			{//enemy within 80, if very close, use melee attack to slap away
				...
				&& !NPC->client->ps.weaponTime //not firing
				&& !PM_InKnockDown(&NPC->client->ps)//not knocked down
				&& !InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, -0.25f))//within 80 and generally behind
			{//enemy within 80, if very close, use melee attack to slap away			
				if (TIMER_Done(NPC, "attackDelay"))
				{//animate me
					int swingAnim;
					swingAnim = Q_irand(BOTH_A7_KICK_B, BOTH_SWEEP_KICK);
					G_Sound(NPC->enemy, G_SoundIndex(va("sound/weapons/melee/punch%d", Q_irand(1, 4))));
					G_AddVoiceEvent(NPC, Q_irand(EV_COMBAT1, EV_COMBAT3), 2000);
					NPC_SetAnim(NPC, SETANIM_BOTH, swingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					if (NPC->client->ps.blockPoints > BLOCKPOINTS_THIRTY)
	...
	if (g_SerenityJediEngineMode->integer && Jedi_CultistDarkForceUser(NPC)
		&& !(ucmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCE_FOCUS | BUTTON_USE | BUTTON_BLOCK))
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		if (NPC->client->ps.torsoAnim == BOTH_A7_SLAP_R ||
	...
	{//we're way out of range
			...
			&& !Q_irand(0, 2))
		{
			if (NPC->enemy
				...
				&& NPC->client->NPC_class != CLASS_KYLE
				&& ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER || NPCInfo->aiFlags & NPCAI_BOSS_SERENITYJEDIENGINE)
					|| NPC->client->NPC_class == CLASS_SHADOWTROOPER)
	...
	{//we're out of striking range and we are allowed to attack
		...
		{//They're throwing their saber, grip them!
			...
			{
				G_AddVoiceEvent(NPC, Q_irand(EV_TAUNT1, EV_TAUNT5), 3000);
				jediSpeechDebounceTime[NPC->client->playerTeam] = NPCInfo->blockedSpeechDebounceTime = level.time + 3000;
				if ((NPCInfo->aiFlags & NPCAI_ROSH))
				{
		...
		else
		{
			...
			else
			{
				...
				else if (!Q_stricmp("MD_Yoda", NPC->NPC_type)
				...
				{//else, randomly try some kind of attack every now and then
						...
						&& (!Q_irand(0, 1) || NPC->s.weapon != WP_SABER))
					{
						if (WP_ForcePowerUsable(NPC, FP_PULL, 0) && !Q_irand(0, 2))
						{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_snoke_cin", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_snoke", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1)
							{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_palpatine", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1)
							{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_sidious_ep2", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1)
							{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_sidious", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1)
							{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_sidious_ep3_red", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1)
							{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_pal_mof", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1)
							{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_emperor", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1)
							{
...
//FIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXMEFIXME
		...
		&& !PM_SaberInSpecialAttack(self->client->ps.torsoAnim))
	{
		...
		if (trace.fraction >= 1.0f && (self->client->NPC_class == CLASS_ALORA) && allowCartWheels)
...
evasionType_t Jedi_SaberBlockGo(gentity_t* self, usercmd_t* cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t* incoming, float dist = 0.0f)
{
	...
	qboolean NPCBlocking = (self->client->ps.ManualBlockingFlags & (1 << MBF_NPCBLOCKING) ? qtrue : qfalse);
	...
	else
	{
		...
		{//either it will miss by a bit (and 25% chance) OR our saber is not in-hand OR saber is off
			if (g_SerenityJediEngineMode->integer)
			{
				if (self->NPC && (NPC->client->ps.blockPoints < BLOCKPOINTS_THIRTY || NPC->client->ps.forcePower < BLOCKPOINTS_HALF))
		...
		if (incoming || !saberBusy || alwaysDodgeOrRoll)
		{
			...
			{//coming from right
				...
				else
				{
					if (g_SerenityJediEngineMode->integer == 2)
					{
						if (self->s.weapon == WP_SABER && NPCBlocking)
						{
							self->client->ps.saberBlocked = BLOCKED_UPPER_RIGHT_MD;
						}
						else
						{
					...
					if (self->client->ps.groundEntityNum != ENTITYNUM_NONE && !NPCBlocking)
					{
						if (zdiff > 5)
						{
							TIMER_Start(self, "duck", Q_irand(500, 1500));
							...
							if (d_JediAI->integer || d_combatinfo->integer)
							{
								gi.Printf("not saberblocking,duck ");
							}
						}
						else
						{
			...
			{//coming from left
				...
				else
				{
					if (g_SerenityJediEngineMode->integer == 2)
					{
						if (self->s.weapon == WP_SABER && NPCBlocking)
						{
							self->client->ps.saberBlocked = BLOCKED_UPPER_LEFT_MD;
						}
						else
						{
			...
			if (rightdot > 8 || (rightdot > 3 && zdiff < -11))//was normalized, 0.2
			{
				if (doDodge)
				{
					...
					else
					{
						if (g_SerenityJediEngineMode->integer)
						{
							if (self->s.weapon == WP_SABER && NPC->health > BLOCKPOINTS_HALF)
							{
								self->client->ps.saberBlocked = BLOCKED_UPPER_RIGHT_MD;
							}
							else
			...
			{//gotta jump!
				...
				{//superjump
					if (self->NPC
						...
						&& !(self->client->ps.forcePowersActive & (1 << FP_RAGE))
						&& !PM_InKnockDown(&self->client->ps) && self->client->NPC_class != CLASS_GUARD)
					{
						if (g_SerenityJediEngineMode->integer)
						{
							if (incoming)
						...
						{
							self->client->ps.forceJumpCharge = 320;
				...
				{//normal jump
					if (self->NPC
						&& !(self->NPC->scriptFlags & SCF_NO_ACROBATICS)
						&& self->client->ps.forceRageRecoveryTime < level.time
						&& !(self->client->ps.forcePowersActive & (1 << FP_RAGE)) && self->client->NPC_class != CLASS_GUARD)
					{
						if (g_SerenityJediEngineMode->integer)
						{
						...
						|| self->client->NPC_class == CLASS_TAVION)
					{
							...
							&& !Q_irand(0, 2))
						{
							...
							{//do the butterfly!
								...
								self->client->ps.forceJumpZStart = self->currentOrigin[2];//so we don't take damage if we land at same height
								self->client->ps.pm_flags |= PMF_JUMPING | PMF_SLOW_MO_FALL;
								self->client->ps.SaberActivateTrail(300);
								{
...
static qboolean Jedi_SaberBlock(void)
{
	...
	{//did some sort of evasion
		...
		{//(not dodge)
			...
			if (d_JediAI->integer || d_combatinfo->integer)
			{
				gi.Printf(S_COLOR_GREEN"Keep parry choice until: %d\n", level.time + parryReCalcTime);
			}
...
qboolean Jedi_EvasionRoll(gentity_t* aiEnt)
{
	...
	gi.trace(&tr, start, NULL, NULL, end, aiEnt->s.number, MASK_NPCSOLID, (EG2_Collision)0, 0);
					...
					{//FIXME: make this a function call?
						...
						else
						{
							...
							else
							{
								...
								{
									NPC->client->ps.forceJumpCharge = 10;
								}
								else
								{
...
static qboolean Jedi_AttackDecide(int enemy_dist)
{
	...
	{//we won a saber lock, press the advantage with an attack!
		int	chance = 0;
		if ((NPCInfo->aiFlags & NPCAI_BOSS_SERENITYJEDIENGINE) || (NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER))
		{//desann and luke
			chance = 20;
		}
		...
		else if (NPC->client->NPC_class == CLASS_GUARD && NPCInfo->rank == RANK_LT_JG)
		{//fencer
			chance = 5;
		}
		else
	...
	if (g_SerenityJediEngineMode->integer)
	{
		if (g_SerenityJediEngineMode->integer == 2)
		{
			if (NPC_IsSaberMaster(NPC) && (NPC->client->ps.blockPoints > BLOCKPOINTS_FULL))
		...
		{
			if (NPC_IsSaberMaster(NPC) && (NPC->client->ps.forcePower > BLOCKPOINTS_FULL))
			{//tavion, fencers, jedi trainer are all good at following up a parry with an attack
				if (PM_SaberInKnockaway(NPC->client->ps.saberMove) && NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN)
				{//try to attack straight from a knockaway
					NPC->client->ps.weaponTime = NPCInfo->shotTime = NPC->attackDebounceTime = 0;
...
static qboolean Jedi_Jump(vec3_t dest, int goalEntNum)
{
	...
	{//a more complicated jump
		...
		if (NPC->currentOrigin[2] > dest[2])
		{
			VectorCopy(NPC->currentOrigin, p1);
			VectorCopy(dest, p2);
		}
		else if (NPC->currentOrigin[2] < dest[2])
		{
			VectorCopy(dest, p1);
...
void NPC_Jedi_Pain(gentity_t* self, gentity_t* inflictor, gentity_t* other, const vec3_t point, int damage, int mod, int hitLoc)
{
	...
	{//back off
		...
		if (self->client->NPC_class == CLASS_DESANN || !Q_stricmp("Yoda", self->NPC_type) || !Q_stricmp("md_Yoda", self->NPC_type))
		{//less for Desann
		...
		{//ouch... maybe switch up which saber power level we're using
			if ((self->client->ps.saberAnimLevel == SS_STRONG)
				...
				&& !(self->client->ps.saberAnimLevel == SS_STAFF)
				&& !(self->client->ps.saberAnimLevel == SS_DUAL)
				&& !(NPC_IsStaffStyle(self))
				&& !(NPC_IsDualStyle(self)))
			{//my saber is not in a parrying position
					...
					if (Q_irand(0, self->NPC->rank) > RANK_ENSIGN)
					{
						if (!Q_irand(0, 1))
						{
							if (other->s.number < MAX_CLIENTS
								&& ((self->NPC->aiFlags & NPCAI_BOSS_CHARACTER || self->NPC->aiFlags & NPCAI_BOSS_SERENITYJEDIENGINE)
									|| self->client->NPC_class == CLASS_SHADOWTROOPER)
...
qboolean Jedi_CanPullBackSaber(gentity_t* self)
{
	...
	if ((self->NPC && (self->NPC->aiFlags & NPCAI_BOSS_CHARACTER || self->NPC->aiFlags & NPCAI_BOSS_SERENITYJEDIENGINE)))
	{
		return qtrue;
	}
...
static void Jedi_Attack(void)
{
	...
	if (NPC->client->ps.saberLockTime > level.time)
	{
		...
		else
		{
			...
			if (NPC->client->NPC_class == CLASS_DESANN || NPC->client->NPC_class == CLASS_SITHLORD || NPC->client->NPC_class == CLASS_VADER)
			{
	...
	if ((NPC->client->NPC_class != CLASS_BOBAFETT && NPC->client->NPC_class != CLASS_MANDALORIAN && NPC->client->NPC_class != CLASS_JANGO)
		...
		&& NPC->client->NPC_class != CLASS_ROCKETTROOPER)
	{
		if (PM_SaberInBrokenParry(NPC->client->ps.saberMove) || NPC->client->ps.saberBlocked == BLOCKED_PARRY_BROKEN || NPC->client->ps.saberBlocked == BLOCKED_ATK_BOUNCE)
		{//just make sure they don't pull their saber to them if they're being blocked
			ucmd.buttons &= ~BUTTON_ATTACK;
		}
	}

	if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || (NPCInfo->aiFlags & NPCAI_BOSS_SERENITYJEDIENGINE) || (NPCInfo->aiFlags & NPCAI_SUBBOSS_CHARACTER) || NPC->s.weapon == WP_SABER)
	{
		if (PM_SaberInBrokenParry(NPC->client->ps.saberMove) || NPC->client->ps.saberBlocked == BLOCKED_PARRY_BROKEN || NPC->client->ps.saberBlocked == BLOCKED_ATK_BOUNCE)
		{//just make sure they don't pull their saber to them if they're being blocked
...
qboolean Jedi_InSpecialMove(void)
{
	...
	if (NPC->client->NPC_class == CLASS_REBORN || NPC->client->NPC_class == CLASS_GALEN)
	{
		if ((NPCInfo->aiFlags & NPCAI_HEAL_ROSH))
		{
...
void NPC_CheckEvasion(void)
{
	...
	{//do we need to do any evasion for other kinds of enemies?
		if (NPC->enemy->client)
		{
			...
			if (NPC->enemy->client->ps.weaponstate == WEAPON_FIRING && InFieldOfVision(NPC->enemy->client->ps.viewangles, 90, ang))
			...
			{// Check for nearby missiles/grenades to evade...
				...
				numEnts = gi.EntitiesInBox(mins, maxs, entityList, MAX_GENTITIES);
