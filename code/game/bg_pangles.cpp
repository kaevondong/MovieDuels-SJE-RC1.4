...
int G_MinGetUpTime(gentity_t* ent)
{
	...
	else if (ent && ent->client && ent->client->NPC_class == CLASS_STORMTROOPER)
	{//stormtroopers are slow to get up
		return 100;
	}
	else if (ent && ent->client && ent->client->NPC_class == CLASS_GUARD)
	{//stormtroopers are slow to get up
		return 100;
	}
	else if (ent && ent->client && ent->client->NPC_class == CLASS_STORMCOMMANDO)
	{//stormtroopers are slow to get up
		return 100;
	}
...
qboolean G_OkayToLean(playerState_t* ps, usercmd_t* cmd, qboolean interruptOkay)
{
	if ((ps->clientNum < MAX_CLIENTS || G_ControlledByPlayer(&g_entities[ps->clientNum]))//player
		...
		&& ((interruptOkay
			&& (PM_DodgeAnim(ps->torsoAnim) || PM_BlockAnim(ps->torsoAnim) || PM_BlockDualAnim(ps->torsoAnim) || PM_BlockStaffAnim(ps->torsoAnim)))//already leaning
...
qboolean G_OkayToBlock(playerState_t* ps, usercmd_t* cmd, qboolean interruptOkay)
{
	if (ps->groundEntityNum != ENTITYNUM_NONE
		&& interruptOkay
		&& !in_camera
		&& (!PM_SaberInMassiveBounce(ps->torsoAnim))
		&& ps->pm_flags & PMF_BLOCK_HELD
		&& ps->forcePower > BLOCKPOINTS_FIVE
		&& ps->blockPoints > BLOCKPOINTS_FIVE
		&& (!(cmd->buttons & (BUTTON_ALT_ATTACK | BUTTON_FORCE_LIGHTNING | BUTTON_USE_FORCE | BUTTON_FORCE_DRAIN | BUTTON_FORCEGRIP))))
	{
		return qtrue;
	}
	return qfalse;
...
void PM_UpdateViewAngles(int saberAnimLevel, playerState_t* ps, usercmd_t* cmd, gentity_t* gent)
{
	...
	//standing block
	if (g_SerenityJediEngineMode->integer && (saberAnimLevel == SS_DUAL)
		&& gent
		&& gent->client
	...
	{//only in the real pmove
		if ((cmd->buttons & BUTTON_BLOCK) && !(cmd->buttons & BUTTON_WALKING))
		{//check blocking
			int anim = -1;

			if (G_OkayToLean(ps, cmd, qtrue) && !PM_WalkingOrRunningAnim(pm->ps->legsAnim))
			{//third person block
				if ((cmd->rightmove))//pushing a direction
				{
					if (cmd->rightmove > 0)
					{//block right
					...
					else if ((cmd->buttons & BUTTON_ATTACK))
					{
				...
				if (anim != -1)
				{
					...
					{//already in a block
					 //use the block pose, don't start it all over again
			...
			{//Only block if holding block button, strafing and not moving forward or back and not jumping
				int leanofs = 0;
				vec3_t	viewangles;
	...
	else if (g_SerenityJediEngineMode->integer && (saberAnimLevel == SS_STAFF)
		&& gent
		&& gent->client
		&& gent->s.weapon == WP_SABER
		&& pm->ps->SaberActive()
