...
extern void WP_SaberStartMissileBlockCheck_MD(gentity_t* self, usercmd_t* ucmd);
...
extern int Manual_RunningAndSaberblocking(gentity_t* defender);
extern int Manual_NPCSaberblocking(gentity_t* defender);
extern int Manual_JKAMode_NPCSaberblocking(gentity_t* defender);
extern qboolean BG_InSlowBounce(playerState_t* ps);
extern qboolean BG_InBlockedAttackBounce(playerState_t* ps);
extern int BG_InGrappleMove(int move);
extern qboolean WalkCheck(gentity_t* self);
extern qboolean PM_SaberInBrokenParry(int move);
extern void SabBeh_AnimateSlowBounce(gentity_t* self, gentity_t* inflictor);
extern void G_Slapdown(gentity_t* self, gentity_t* attacker, const vec3_t pushDir, float strength, qboolean breakSaberLock);
extern void BG_ReduceSaberMishapLevel(playerState_t* ps);
extern qboolean PM_SaberInAttackPure(int move);
extern qboolean PM_SaberInTransitionAny(int move);
extern qboolean G_OkayToBlock(playerState_t* ps, usercmd_t* cmd, qboolean interruptOkay);
...
extern void AddFatigueMeleeBonus(gentity_t* attacker, gentity_t* victim);
...
qboolean G_ControlledByNPC(gentity_t* self)
{
...
void ClientTimerActions(gentity_t* ent, int msec)
{
	...
	while (client->timeResidual >= 1000)
	{
		...
		{
			if (ent->client->ps.saberAttackChainCount > MISHAPLEVEL_NONE && (ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent))
			...
			else if (ent->client->ps.saberAttackChainCount > MISHAPLEVEL_NONE
				&& !BG_InSlowBounce(&ent->client->ps)
				&& !PM_SaberInBrokenParry(ent->client->ps.saberMove)
				&& !PM_SaberInAttackPure(ent->client->ps.saberMove)
				&& !PM_SaberInAttack(ent->client->ps.saberMove)
			...
			if (g_SerenityJediEngineMode->integer == 2
				&& !PM_SaberInAttack(ent->client->ps.saberMove)&& !(ent->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING)))
...
qboolean WP_AbsorbKick(gentity_t* hitEnt, gentity_t* pusher, const vec3_t pushDir)
{
	vec3_t pLAngles, pLFwd;

	if (hitEnt->client->ps.forcePowerLevel[FP_SABER_DEFENSE] < FORCE_LEVEL_1)
	{//only force-users
		return qfalse;
	}

	if (hitEnt->health <= 1
		|| BG_InKnockDown(hitEnt->client->ps.legsAnim)
		|| BG_InKnockDown(hitEnt->client->ps.torsoAnim)
		|| PM_InRoll(&hitEnt->client->ps)
		|| PM_SuperBreakLoseAnim(hitEnt->client->ps.torsoAnim)
		|| PM_SuperBreakWinAnim(hitEnt->client->ps.torsoAnim)
		|| PM_SaberInSpecialAttack(hitEnt->client->ps.torsoAnim)
		|| PM_SaberInBounce(hitEnt->client->ps.saberMove)
		|| PM_SaberInKnockaway(hitEnt->client->ps.saberMove)
		|| PM_SaberInBrokenParry(hitEnt->client->ps.saberMove))
	{
		return qfalse;
	}
	...
	if ((hitEnt->s.number < MAX_CLIENTS || G_ControlledByPlayer(hitEnt))) //player only restrictions
	{
		if (hitEnt->client->ps.saberAttackChainCount >= MISHAPLEVEL_TEN)
		{
			return qfalse;
		}
		...
		if (hitEnt->client->ps.forcePower < FATIGUE_DODGEING)
		{//must have enough force power
			return qfalse;
		}

		if (hitEnt->client->ps.blockPoints < FATIGUE_DODGEING && g_SerenityJediEngineMode->integer == 2)
		{//must have enough block power
			return qfalse;
		}
	}

	if (BG_InGrappleMove(hitEnt->client->ps.torsoAnim))
	{//can't block while grappling
		return qfalse;
	}

	if (hitEnt->client->ps.groundEntityNum == ENTITYNUM_NONE)
	{//can't absorb kicks while in midair
		return qfalse;
	}

	VectorSet(pLAngles, 0, hitEnt->client->ps.viewangles[YAW], 0);
	AngleVectors(pLAngles, pLFwd, NULL, NULL);

	if (DotProduct(pLFwd, pushDir) > 0.2f)
	{//not hit in the front, can't absorb it.
		return qfalse;
	}

	if (pusher->client->ps.legsAnim == BOTH_A7_KICK_F || pusher->client->ps.legsAnim == BOTH_KICK_F_MD
		...
		|| pusher->client->ps.torsoAnim == BOTH_TUSKENATTACK3)
	{
		if (g_SerenityJediEngineMode->integer)
		{
			if (g_SerenityJediEngineMode->integer == 2)
			{//Advanced MD Mode
				if (hitEnt->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING) && (hitEnt->s.number < MAX_CLIENTS || G_ControlledByPlayer(hitEnt))) //player only
				{
					G_Stagger(hitEnt, pusher, qtrue);
					//NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_FLIP_BACK2, BOTH_FLIP_BACK3), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BP_ABSORB);
				}
				else if (hitEnt->s.clientNum >= MAX_CLIENTS && !G_ControlledByPlayer(hitEnt) && hitEnt->client->ps.blockPoints > 50) //NPC only
				{
					G_Stagger(hitEnt, pusher, qtrue);
					//NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_FLIP_BACK1, BOTH_FLIP_BACK2), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BP_ABSORB);
					WP_DeactivateSaber(hitEnt, qtrue);
				}
				else
				{
					NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_KNOCKDOWN1, BOTH_KNOCKDOWN2), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BLOCKPOINTDRAIN);
					AddFatigueMeleeBonus(pusher, hitEnt);

					if (d_slowmoaction->integer && (pusher->s.number < MAX_CLIENTS || G_ControlledByPlayer(pusher)))
					{
						if (g_SerenityJediEngineMode->integer == 2)
						{
			...
			{//MD MODE
				if (hitEnt->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING) && (hitEnt->s.number < MAX_CLIENTS || G_ControlledByPlayer(hitEnt))) //player only
				{
					G_Stagger(hitEnt, pusher, qtrue);
					//NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_FLIP_BACK1, BOTH_FLIP_BACK2), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_ForcePowerDrain(hitEnt, FP_SABER_DEFENSE, FATIGUE_KICKHIT_ABSORB);
				}
				else if (hitEnt->s.clientNum >= MAX_CLIENTS && !G_ControlledByPlayer(hitEnt) && hitEnt->client->ps.forcePower > 85) //NPC only
				{
					G_Stagger(hitEnt, pusher, qtrue);
					//NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_FLIP_BACK1, BOTH_FLIP_BACK2), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_ForcePowerDrain(hitEnt, FP_SABER_DEFENSE, FATIGUE_KICKHIT_ABSORB);
		...
		{//JKA Mode
			if (hitEnt->client->ps.forcePower > 85) //More than 85 FP
			{
				G_Stagger(hitEnt, pusher, qtrue);
				//NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_FLIP_BACK1, BOTH_FLIP_BACK2), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				WP_ForcePowerDrain(hitEnt, FP_SABER_DEFENSE, FATIGUE_KICKHIT_ABSORB);
			...
			{
				NPC_SetAnim(hitEnt, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				WP_ForcePowerDrain(hitEnt, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
				AddFatigueMeleeBonus(pusher, hitEnt);
			}
		}
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_B)
	{
		if (g_SerenityJediEngineMode->integer)
		{
			...
			{//Advanced MD Mode
				...
				else if (hitEnt->s.clientNum >= MAX_CLIENTS && !G_ControlledByPlayer(hitEnt) && hitEnt->client->ps.blockPoints > 75) //NPC only
				{
					NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_FLIP_BACK1, BOTH_FLIP_BACK2), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BP_ABSORB);
				}
				...
				{
					NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_KNOCKDOWN1, BOTH_KNOCKDOWN5), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BLOCKPOINTDRAIN);
					AddFatigueMeleeBonus(pusher, hitEnt);
				}
			}
	...
	else if (pusher->client->ps.legsAnim == BOTH_SWEEP_KICK)
	{
		if (g_SerenityJediEngineMode->integer)
		{
			...
			{//Advanced MD Mode
				...
				{
					NPC_SetAnim(hitEnt, SETANIM_BOTH, BOTH_KNOCKDOWN3, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BLOCKPOINTDRAIN);
					AddFatigueMeleeBonus(pusher, hitEnt);
		...
		|| pusher->client->ps.torsoAnim == BOTH_A7_SLAP_R)
	{
		if (g_SerenityJediEngineMode->integer)
		{
			...
			{//Advanced MD Mode
				...
				{
					NPC_SetAnim(hitEnt, SETANIM_BOTH, BOTH_SLAPDOWNRIGHT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BLOCKPOINTDRAIN);
					AddFatigueMeleeBonus(pusher, hitEnt);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_SLAP_R)
	{
		if (g_SerenityJediEngineMode->integer)
		{
			if (g_SerenityJediEngineMode->integer == 2)
		...
		|| pusher->client->ps.torsoAnim == BOTH_A7_SLAP_L)
	{
		if (g_SerenityJediEngineMode->integer)
		{
			...
			{//Advanced MD Mode
				...
				{
					NPC_SetAnim(hitEnt, SETANIM_BOTH, BOTH_SLAPDOWNLEFT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BLOCKPOINTDRAIN);
					AddFatigueMeleeBonus(pusher, hitEnt);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_SLAP_L)
	{
		if (g_SerenityJediEngineMode->integer)
		{
			if (g_SerenityJediEngineMode->integer == 2)
	...
	else if (pusher->client->ps.torsoAnim == BOTH_TUSKENLUNGE1)
	{
		if (g_SerenityJediEngineMode->integer)
		{
			...
			{//Advanced MD Mode
				...
				{
					NPC_SetAnim(hitEnt, SETANIM_BOTH, Q_irand(BOTH_SLAPDOWNRIGHT, BOTH_SLAPDOWNLEFT), SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BLOCKPOINTDRAIN);
					AddFatigueMeleeBonus(pusher, hitEnt);
				}
			}
		...
		G_Sound(hitEnt, G_SoundIndex(va("sound/weapons/melee/punch%d", Q_irand(1, 4))));
		if (pusher->client->ps.legsAnim == BOTH_TUSKENLUNGE1)
		{
			G_Sound(pusher, G_SoundIndex(va("sound/weapons/melee/swing%d", Q_irand(1, 4))));
		}
		hitEnt->client->ps.saberMove = LS_NONE;
	...
	else if (pusher->client->ps.torsoAnim == BOTH_A7_HILT)//for the hiltbash
	{
		if (g_SerenityJediEngineMode->integer)
		{
			...
			{//Advanced MD Mode
				...
				{
					NPC_SetAnim(hitEnt, SETANIM_BOTH, BOTH_BASHED1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					WP_BlockPointsDrain(hitEnt, FATIGUE_BLOCKPOINTDRAIN);
					AddFatigueMeleeBonus(pusher, hitEnt);
				}
			}
...
{
	vec3_t	traceOrg, traceEnd, kickMins = { -2,-2,-2 }, kickMaxs = { 4,4,4 };
	trace_t	trace;
	...
	gi.trace(&trace, traceOrg, kickMins, kickMaxs, traceEnd, ent->s.number, MASK_SHOT, (EG2_Collision)0, 0);
			...
			if (hitEnt->client)
			{
					...
					&& TIMER_Done(hitEnt, "kickedDebounce") && G_CanBeEnemy(ent, hitEnt))
				{
						 ...
						 //player falls off a cliff or something.
							if (hitEnt->client)
							{
								...
								{//kicks do fatigue damage if they were stunned.
									if (g_SerenityJediEngineMode->integer == 2)
									{
										WP_BlockPointsDrain(hitEnt, BLOCKPOINTS_TEN);
									}
									else
									{
										WP_ForcePowerDrain(hitEnt, FP_SABER_DEFENSE, BLOCKPOINTS_TEN);
									}
								}
							}
							G_Throw(hitEnt, kickDir, 80);
							hitEnt->client->ps.saberBounceMove = LS_READY;
							return (hitEnt);
						}
						G_Damage(hitEnt, ent, ent, kickDir, trace.endpos, kickDamage * 0.2f, DAMAGE_NO_KNOCKBACK | DAMAGE_NO_KILL, MOD_MELEE);
					}
					...
					{//we kicked a dead guy
						if (!(hitEnt->flags & FL_NO_KNOCKBACK))
						{
							G_Throw(hitEnt, kickDir, kickPush * 2);
						}
						G_ThrownDeathAnimForDeathAnim(hitEnt, trace.endpos);
					}
					else
	...
	{//backflip attack
		...
		{//middle of anim
			...
			{//still on ground?
				...
				if ((g_SerenityJediEngineMode->integer == 2) && ent->client->ps.forcePowerLevel[FP_LEVITATION] < FORCE_LEVEL_3)
				{//short burst
	...
	{//can't move, check for damage frame
		...
		else
		{
			...
			case BOTH_A7_SLAP_R:
				if (level.framenum & 1)
				{//back
					int handBolt = (ent->client->ps.legsAnim == BOTH_A7_SLAP_R) ? ent->handRBolt : ent->handLBolt;//mirrored anims
					doKick = qtrue;
					kickDist = 80;
			...
			case BOTH_A7_SLAP_L:
				if (level.framenum & 1)
				{//back
					int handBolt = (ent->client->ps.legsAnim == BOTH_A7_SLAP_L) ? ent->handRBolt : ent->handLBolt;//mirrored anims
					doKick = qtrue;
					kickDist = 80;
					if (handBolt != -1)//changed to accomadate teh new hand hand anim
					{//actually trace to a bolt
...
{
	if (ent->health < 1 ||
		ent->client->VaderBreathTime > cg.time ||
	...
	if (PM_CrouchAnim(ent->client->ps.legsAnim) ||
		...
		PM_LungeAnim(ent->client->ps.torsoAnim) ||
		PM_RollingAnim(ent->client->ps.legsAnim))
	{
		return;
	}
	...
	else if (ent->client->NPC_class == CLASS_DESANN)
	{
		if (ent && !Q_stricmp("md_vader_ep3", ent->NPC_type) ||
			ent && !Q_stricmp("md_vader_anh", ent->NPC_type) ||
			ent && !Q_stricmp("md_vad_tfu", ent->NPC_type) ||
	...
	if ((PM_SaberInAttack(ent->client->ps.saberMove)) || (PM_RunningAnim(ent->client->ps.legsAnim)))
	{
		ent->client->VaderBreathTime = cg.time + 4000; // every 3 seconds.
	}
	else if (in_camera)
	{
		ent->client->VaderBreathTime = cg.time + 4000; // every 6 seconds.
	}
	else if (gi.VoiceVolume[ent->s.number] > 0)
	{
		ent->client->VaderBreathTime = cg.time + 10000; // every 6 seconds.
	}
	else
	{
		ent->client->VaderBreathTime = cg.time + 6000; // every 6 seconds.
	}
}

extern vmCvar_t	cg_setVaderBreath;
extern vmCvar_t	cg_setVaderBreathdamaged;
void CG_SetVaderBreath(gentity_t* ent)
{
...
void CG_SetVaderBreathDamaged(gentity_t* ent)
{
	if (ent->health < 1 || ent->client->VaderBreathTime > cg.time)
	{
		return;
...
void ClientThink_real(gentity_t* ent, usercmd_t* ucmd)
{
	...
	if (g_SerenityJediEngineMode->integer &&
		...
		!PM_Saberinstab(ent->client->ps.saberMove))
	{
		if (Manual_RunningAndSaberblocking(ent))
		{
			if (!(ent->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING)))
			{
				...
				ent->client->ps.ManualBlockingTime = level.time; //Blocking time 1 on
			}
		}
		else if (Manual_Saberblocking(ent))
		{
			...
			if (client->usercmd.buttons & BUTTON_ATTACK	&& ent->client->ps.weaponTime <= 0)
			{
				if (!(ent->client->ps.ManualBlockingFlags & (1 << MBF_PROJBLOCKING)))
				{
					ent->client->ps.ManualBlockingFlags |= (1 << MBF_PROJBLOCKING);
					ent->client->ps.ManualMBlockingTime = level.time;
				}
				client->usercmd.buttons &= ~BUTTON_ATTACK;

				if (g_spskill->integer == 0)
				{
					if (ent->client->ps.ManualblockStartTime <= 0 &&
						level.time - ent->client->ps.ManualblockLastStartTime >= g_SaberMBlockingTimerwaitEasy->integer)
					{// They just pressed block. Mark the time... g_SaberPerfectBlockingTimer->integer wait between allowed presses.
						ent->client->ps.ManualblockStartTime = level.time; //Blocking 2
						ent->client->ps.ManualblockLastStartTime = level.time; //Blocking 3
					...
					{
						if (level.time - ent->client->ps.ManualblockStartTime >= g_SaberPerfectBlockingTimerEasy->integer) //Blocking 3
						{// When block was pressed, wait g_SaberPerfectBlockingTimer->integer before letting go of block.
							ent->client->ps.ManualblockStartTime = 0; //Blocking 2
							ent->client->ps.ManualBlockingFlags &= ~(1 << MBF_MBLOCKING);
							if (d_combatinfo->integer)
							{
				...
				{
					if (ent->client->ps.ManualblockStartTime <= 0 &&
						level.time - ent->client->ps.ManualblockLastStartTime >= g_SaberMBlockingTimerwaitNormal->integer)
					{// They just pressed block. Mark the time... g_SaberPerfectBlockingTimer->integer wait between allowed presses.
						ent->client->ps.ManualblockStartTime = level.time; //Blocking 2
						ent->client->ps.ManualblockLastStartTime = level.time; //Blocking 3
					...
					{
						if (level.time - ent->client->ps.ManualblockStartTime >= g_SaberPerfectBlockingTimerNormal->integer) //Blocking 3
						{// When block was pressed, wait g_SaberPerfectBlockingTimer->integer before letting go of block.
							ent->client->ps.ManualblockStartTime = 0; //Blocking 2
							ent->client->ps.ManualBlockingFlags &= ~(1 << MBF_MBLOCKING);
							if (d_combatinfo->integer)
				...
				{
					if (ent->client->ps.ManualblockStartTime <= 0 &&
						level.time - ent->client->ps.ManualblockLastStartTime >= g_SaberMBlockingTimerwaitHard->integer)
					{// They just pressed block. Mark the time... g_SaberPerfectBlockingTimer->integer wait between allowed presses.
						ent->client->ps.ManualblockStartTime = level.time; //Blocking 2
						ent->client->ps.ManualblockLastStartTime = level.time; //Blocking 3
					...
					{
						if (level.time - ent->client->ps.ManualblockStartTime >= g_SaberPerfectBlockingTimerHard->integer) //Blocking 3
						{// When block was pressed, wait g_SaberPerfectBlockingTimer->integer before letting go of block.
							ent->client->ps.ManualblockStartTime = 0; //Blocking 2
							ent->client->ps.ManualBlockingFlags &= ~(1 << MBF_MBLOCKING);
							if (d_combatinfo->integer)
		...
		else
		{
			...
			ent->client->ps.ManualBlockingFlags &= ~(1 << MBF_MBLOCKING);
			ent->client->ps.userInt3 &= ~(1 << FLAG_BLOCKING);
			ent->client->ps.ManualBlockingTime = 0; //Blocking time 1 on
		...
		&& !PM_Saberinstab(ent->client->ps.saberMove))
	{
		...
		{
			if (!(ent->client->ps.ManualBlockingFlags & (1 << MBF_NPCBLOCKING)))
			{
				ent->client->ps.ManualBlockingFlags |= (1 << MBF_NPCBLOCKING);
				ent->client->ps.userInt3 |= (1 << FLAG_NPCBLOCKING);
			}
		}
		else
		{
		...
		&& !PM_Saberinstab(ent->client->ps.saberMove))
	{
		...
		{
			if (!(ent->client->ps.ManualBlockingFlags & (1 << MBF_JKAMODENPCBLOCKING)))
			{
				ent->client->ps.ManualBlockingFlags |= (1 << MBF_JKAMODENPCBLOCKING);
			}
		}
		else
		{
	...
	if ((ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent)))
	{
		if (IsRespecting(ent))
		{
			ent->client->ps.respectingtime = level.time;

			if (ent->client->ps.respectingstartTime <= 0 && level.time - ent->client->ps.respectinglaststartTime >= 8000)
			{// They just pressed respect. Mark the time... 8000 wait between allowed Respecting.
				ent->client->ps.respectingstartTime = level.time;
				ent->client->ps.respectinglaststartTime = level.time;

				if (!(ent->client->ps.communicatingflags & (1 << RESPECTING)))
				{
					ent->client->ps.communicatingflags |= (1 << RESPECTING);
				}
			}
			else
			{
				if (level.time - ent->client->ps.respectingstartTime >= 3000)
				{// When respect was pressed, wait 3000 before letting go of respect.
					ent->client->ps.respectingstartTime = 0;
					ent->client->ps.communicatingflags &= ~(1 << RESPECTING);
				}
			}
		}
		...
		{
			ent->client->ps.gesturingtime = level.time;

			if (ent->client->ps.gesturingstartTime <= 0 && level.time - ent->client->ps.gesturinglaststartTime >= 8000)
			{// They just pressed respect. Mark the time... 8000 wait between allowed Respecting.
				ent->client->ps.gesturingstartTime = level.time;
				ent->client->ps.gesturinglaststartTime = level.time;

				if (!(ent->client->ps.communicatingflags & (1 << GESTURING)))
				{
					ent->client->ps.communicatingflags |= (1 << GESTURING);
				}
			}
			else
			{
				if (level.time - ent->client->ps.gesturingstartTime >= 3000)
				{// When respect was pressed, wait 3000 before letting go of respect.
					ent->client->ps.gesturingstartTime = 0;
					ent->client->ps.communicatingflags &= ~(1 << GESTURING);
				}
			}
		}
		...
		{
			ent->client->ps.surrendertimeplayer = level.time;

			if (ent->client->ps.surrenderstartTime <= 0 && level.time - ent->client->ps.surrenderlaststartTime >= 8000)
			{// They just pressed surrender. Mark the time... 8000 wait between allowed surrender.
				ent->client->ps.surrenderstartTime = level.time;
				ent->client->ps.surrenderlaststartTime = level.time;

				if (!(ent->client->ps.communicatingflags & (1 << SURRENDERING)))
				{
					ent->client->ps.communicatingflags |= (1 << SURRENDERING);
				}
			}
			else
			{
				if (level.time - ent->client->ps.surrenderstartTime >= 3000)
				{// When respect was pressed, wait 3000 before letting go of respect.
					ent->client->ps.surrenderstartTime = 0;
					ent->client->ps.communicatingflags &= ~(1 << SURRENDERING);
				}
			}
		}
		...
		{
			ent->client->ps.dashtimeplayer = level.time;

			if (ent->client->ps.dashstartTime <= 0 && level.time - ent->client->ps.dashlaststartTime >= 5000)
			{// They just pressed dash. Mark the time... 8000 wait between allowed dash.
				ent->client->ps.dashstartTime = level.time;
				ent->client->ps.dashlaststartTime = level.time;

				if (!(ent->client->ps.communicatingflags & (1 << DASHING)))
				{
					ent->client->ps.communicatingflags |= (1 << DASHING);
				}
			}
			else
			{
				if (level.time - ent->client->ps.dashstartTime >= 1500)
				{// When respect was pressed, wait 3000 before letting go of respect.
					ent->client->ps.dashstartTime = 0;
					ent->client->ps.communicatingflags &= ~(1 << DASHING);
				}
			}
		}
		...
		{
			ent->client->ps.respectingtime = 0;
			ent->client->ps.gesturingtime = 0;
			ent->client->ps.surrendertimeplayer = 0;
			ent->client->ps.communicatingflags &= ~(1 << SURRENDERING);
			ent->client->ps.communicatingflags &= ~(1 << RESPECTING);
			ent->client->ps.communicatingflags &= ~(1 << GESTURING);
			ent->client->ps.communicatingflags &= ~(1 << DASHING);
		...
		|| (ent->NPC && G_JediInNormalAI(ent)))//NPC jedi not in a special AI mode
	{
		if (g_SerenityJediEngineMode->integer && (ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent)))
		{
			WP_SaberStartMissileBlockCheck_MD(ent, ucmd);
		}
		else
		{
