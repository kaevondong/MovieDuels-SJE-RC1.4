...
saberMoveData_t	saberMoveData[LS_MOVE_MAX] = {//							NB:randomized
	...
	{"StfKickFwdAir2",BOTH_FLYING_KICK,  Q_R,	Q_R,	AFLAG_ACTIVE,	100,	BLK_TIGHT,	LS_READY,		LS_S_R2L,		200 },	// LS_FLYING_KICK
	...
	{ "Knock UR_MD", BOTH_K1_S1_TR_MD,	Q_R,	Q_TR,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BL2TR,		LS_T1_TR__R,		150 },	// LS_K1_TR_MD,
	{ "Knock UR_PB", BOTH_K1_S1_TR_PB,   Q_R,	Q_TR,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BL2TR,		LS_T1_TR__R,	    150 },	// LS_K1_TR_PB,
	{ "Knock UL",	BOTH_K1_S1_TL,		Q_R,	Q_TL,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BR2TL,		LS_T1_TL__L,		150 },	// LS_K1_TL,
	{ "Knock UL_MD", BOTH_K1_S1_TL_MD,	Q_R,	Q_TL,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BR2TL,		LS_T1_TL__L,		150 },	// LS_K1_TL_MD,
	{ "Knock UL_PB", BOTH_K1_S1_TL_PB,	Q_R,	Q_TL,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BR2TL,		LS_T1_TL__L,	    150 },	// LS_K1_TL_PB,
	...
	// Parry MD Mode
	{ "Parry Top_MD",BOTH_P1_S1_T__MD,		Q_R,	Q_T,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BL2TR,		LS_A_T2B,		150 },	// LS_PARRY_UP,
	{ "Parry UR_MD", BOTH_P1_S1_TR_MD,		Q_R,	Q_TL,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BL2TR,		LS_A_TR2BL,		150 },	// LS_PARRY_UR,
	{ "Parry UL_MD", BOTH_P1_S1_TL_MD,		Q_R,	Q_TR,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BR2TL,		LS_A_TL2BR,		150 },	// LS_PARRY_UL,
	{ "Parry LR_MD", BOTH_P1_S1_BR_MD,		Q_R,	Q_BR,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_TL2BR,		LS_A_BR2TL,		150 },	// LS_PARRY_LR,
	{ "Parry LL_MD", BOTH_P1_S1_BL_MD,		Q_R,	Q_BL,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_TR2BL,		LS_A_BL2TR,		150 },	// LS_PARRY_LL
...
qboolean PM_SaberDoDamageAnim(int anim)
{
	switch (anim)
	{
	case BOTH_A2_STABBACK1:
...
qboolean PM_SaberInnonblockableAttack(int anim)
{
	switch (anim)
	{
	case BOTH_ATTACK_BACK:
...
qboolean PM_SaberInReflect( int move )
{
	...
	case LS_REFLECT_LR:
	case LS_REFLECT_LL:
	case LS_REFLECT_UP_MD:
	case LS_REFLECT_UR_MD:
	case LS_REFLECT_UL_MD:
	case LS_REFLECT_LR_MD:
	case LS_REFLECT_LL_MD:
		return qtrue;
		break;
	}
	return qfalse;
...
qboolean PM_KnockAwayStaffAndDuels(int move)
{
	switch (move)
	{
	case BOTH_K6_S6_BL:
...
saberMoveName_t PM_KnockawayForParry(int move)
{
	...
	{
	case BLOCKED_TOP:
	case BLOCKED_TOP_MD://LS_PARRY_UP:
		return LS_K1_T_;//push up
		break;
	case BLOCKED_UPPER_RIGHT:
	case BLOCKED_UPPER_RIGHT_MD://LS_PARRY_UR:
	default://case LS_READY:
		return LS_K1_TR;//push up, slightly to right
		break;
	case BLOCKED_UPPER_LEFT:
	case BLOCKED_UPPER_LEFT_MD://LS_PARRY_UL:
		return LS_K1_TL;//push up and to left
		break;
	case BLOCKED_LOWER_RIGHT:
	case BLOCKED_LOWER_RIGHT_MD://LS_PARRY_LR:
		return LS_K1_BR;//push down and to left
		break;
	case BLOCKED_LOWER_LEFT:
	case BLOCKED_LOWER_LEFT_MD://LS_PARRY_LL:
...
saberMoveName_t PM_KnockawayForParry_AMD(int move)
{
	switch (move)
	{
	case BLOCKED_TOP://LS_PARRY_UP:
	case BLOCKED_TOP_MD://LS_PARRY_UP_MD:
		return LS_K1_T_;//push up
		break;
	case BLOCKED_UPPER_RIGHT://LS_PARRY_UR:
	case BLOCKED_UPPER_RIGHT_MD://LS_PARRY_UR_MD:
	default://case LS_READY:
		return LS_K1_TR_MD;//push up, slightly to right
		Com_Printf(S_COLOR_RED"player blocked using the default PM_KnockawayForParry_AMD \n");
		break;
	case BLOCKED_UPPER_LEFT://LS_PARRY_UL:
	case BLOCKED_UPPER_LEFT_MD://LS_PARRY_UL_MD:
		return LS_K1_TL_MD;//push up and to left
		break;
	case BLOCKED_LOWER_RIGHT://LS_PARRY_LR:
	case BLOCKED_LOWER_RIGHT_MD://LS_PARRY_LR_MD:
		return LS_K1_BR;//push down and to left
		break;
	case BLOCKED_LOWER_LEFT://LS_PARRY_LL:
	case BLOCKED_LOWER_LEFT_MD://LS_PARRY_LL_MD:
		return LS_K1_BL;//push down and to right
		break;
	}
}

int G_KnockawayForParry_AMD(int move)
{
	switch (move)
	{
	case LS_PARRY_UP_MD:
	...
	default:
		return LS_K1_TR_MD;//push up, slightly to right
		Com_Printf(S_COLOR_RED"player blocked using the default G_KnockawayForParry_AMD \n");
		break;
	case LS_PARRY_UL_MD:
	case LS_PARRY_UL:
		return LS_K1_TL_MD;//push up and to left
...
saberMoveName_t PM_KnockawayForParry_MD(int move)
{
	switch (move)
	{
	case BLOCKED_TOP://LS_PARRY_UP:
...
int G_KnockawayForParry_MD(int move)
{
	switch (move)
	{
	case LS_PARRY_UP_MD:
...
// MD ASSISTED BLOCK //
saberMoveName_t PM_KnockawayForParry_PB(int move)
{
	switch (move)
	{
	case BLOCKED_TOP://LS_PARRY_UP:
	...
	case BLOCKED_UPPER_RIGHT_MD://LS_PARRY_UR_MD:
	default://case LS_READY:
		return LS_K1_TR_PB;//push up, slightly to right
		break;
	case BLOCKED_UPPER_LEFT://LS_PARRY_UL:
	case BLOCKED_UPPER_LEFT_MD://LS_PARRY_UL_MD:
		return LS_K1_TL_PB;//push up and to left
		break;
	case BLOCKED_LOWER_RIGHT://LS_PARRY_LR:
	case BLOCKED_LOWER_RIGHT_MD://LS_PARRY_LR_MD:
		return LS_K1_BR;//push down and to left
...
int G_KnockawayForParry_PB(int move)
{
	switch (move)
	{
	case LS_PARRY_UP_MD:
...
qboolean PM_SaberKataDone(int curmove = LS_NONE, int newmove = LS_NONE)
{
	if (g_SerenityJediEngineMode->integer == 2)
	{
		...
		{//FIXME: have chainAngle influence fast and medium chains as well?
			...
			if ((pm->ps->saberAnimLevel == FORCE_LEVEL_2 || pm->ps->saberAnimLevel == SS_DUAL) && pm->ps->saberAttackChainCount > Q_irand(MISHAPLEVEL_RUNINACCURACY, MISHAPLEVEL_LIGHT))
			{
				return qtrue;
			}
		}
		...
		{//okay to do backstabs with this saber
			...
			{//only when on ground
				...
				{//enemy not a client or is a client and on ground
					if ( dot < -0.75f
						&& enemyDist < 128
						&& (pm->ps->saberAnimLevel == SS_FAST || pm->ps->saberAnimLevel == SS_STAFF || (pm->gent->client &&(pm->gent->client->NPC_class == CLASS_TAVION || pm->gent->client->NPC_class == CLASS_GALEN || pm->gent->client->NPC_class == CLASS_ALORA)&&Q_irand(0,2))) )
					{//fast back-stab
...
qboolean PM_CheckLungeAttackMove(void)
{
	...
	//do normal checks
	if ((g_SerenityJediEngineMode->integer == 2 && pm->ps->saberAnimLevel == SS_MEDIUM)
		|| pm->ps->saberAnimLevel == SS_FAST//fast
	...
	{//alt+back+attack using fast, dual or staff attacks
		...
		{//player
			if (G_TryingLungeAttack(pm->gent, &pm->cmd)
				&& G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER_FB))//have enough force power to pull it off
			{
				return qtrue;
			}
		}
...
saberMoveName_t PM_CheckPlayerAttackFromParry( int curmove )
{
	if ( pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer() )
	{
		...
		if (curmove >= LS_PARRY_UP_MD && curmove <= LS_REFLECT_LL_MD)
		{//in a parry
			switch (saberMoveData[curmove].endQuad)
			{
			case Q_T:
	...
	{//not moving left or right
						...
						{//FIXME: or just trace for a valid enemy standing behind me?  And no enemy in front?
							...
							{//enemy is behind me
								if ( dot < -0.75f
									...
									&& (pm->ps->saberAnimLevel == SS_FAST || pm->ps->saberAnimLevel == SS_STAFF || (pm->gent->client &&(pm->gent->client->NPC_class == CLASS_TAVION || pm->gent->client->NPC_class == CLASS_GALEN ||pm->gent->client->NPC_class == CLASS_ALORA)&&Q_irand(0,1))) )
							...
							{//enemy in front
								...
								if (((pm->ps->saberAnimLevel == FORCE_LEVEL_1 ||
									...
									pm->gent->client->NPC_class == CLASS_GALEN ||
									(pm->gent->client->NPC_class == CLASS_VADER && !Q_irand(0, 3)) ||
		...
		{//not moving in any direction
			...
			{//bounces should go to their default attack if you don't specify a direction but are attacking
				...
				|| ( curmove >= LS_PARRY_UP_MD && curmove <= LS_REFLECT_LL_MD ))
...
void PM_SaberStartTransAnimMD(int saberAnimLevel, int anim, float* animSpeed, gentity_t* gent, int fatigued)
{
	if (g_SerenityJediEngineMode->integer)
	{
		...
		{
			char	buf[128];
			float	saberanimscale  = 0.9f;
			float	redanimscale    = 1.0f;
			float	dualanimscale   = 0.9f;
			float	staffanimscale  = 0.9f;
			float	blueanimscale   = 1.0f;
			float	mediumanimscale = 1.0f;
			float	tavionanimscale = 1.0f;
			float	heavyanimscale  = 1.0f;
			...
			if (gent && gent->client)
			{
				if ((gent->client->ps.weapon == WP_SABER) && (PM_InSaberAnim(anim)))
				{
					if (saberAnimLevel == SS_FAST)
					{
						*animSpeed *= 0.85f;
			...
			if (saberAnimLevel == SS_STRONG || saberAnimLevel == SS_DESANN)
			{
			...
			else if ((fatigued & (1 << FLAG_BLOCKDRAINED)) && anim >= BOTH_A1_T__B_ && anim <= BOTH_ROLL_STAB
				//not a wall run move
				&& anim != BOTH_FORCEWALLRELEASE_FORWARD && anim != BOTH_FORCEWALLRUNFLIP_START
				&& anim != BOTH_FORCEWALLRUNFLIP_END
				&& anim != BOTH_JUMPFLIPSTABDOWN
			...
			else if ((fatigued & (1 << FLAG_ATTACKFATIGUE)))
			{
				*animSpeed *= 0.8f;
			}
...
//=================================
	...
	qboolean HoldingBlock = (gent->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING) ? qtrue : qfalse);
	qboolean ActiveBlocking = (gent->client->ps.ManualBlockingFlags & (1 << MBF_PROJBLOCKING) ? qtrue : qfalse);
	...
	if (!PM_InKataAnim(anim))
	{//make smooth animations				
		if ((ActiveBlocking && anim == LS_READY))
		{
			blendTime *= 1.8f;
		}
	}
	...
	{
		PM_SaberStartTransAnimMD(gent->client->ps.saberAnimLevel, anim, &timeScaleMod, gent, gent->userInt3);
	}
	else
	{
		...
		gi.WE_IsOutside(ent->currentOrigin))
	{
		if (Q_stricmp(level.mapname, "t2_wedge") != 0)
		{
			...
			if (gi.WE_GetWindVector(windDir, ent->currentOrigin))
			{
				...
				{
					if (PM_StandingidleAnim(ent->client->ps.legsAnim))
					{
						...
						else
						{
							...
							else
							{
								if (!IsSurrendering(pm->gent))
								{
									if (pm->ps->saberAnimLevel == SS_STAFF)
									{
										PM_SetAnim(pm, SETANIM_TORSO, BOTH_SABER_IDLE_STANCE_STAFF, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
									}
									else if (pm->ps->saberAnimLevel == SS_DUAL)
									{
										PM_SetAnim(pm, SETANIM_TORSO, BOTH_SABER_IDLE_STANCE_DUAL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
									}
									else
									{
										PM_SetAnim(pm, SETANIM_TORSO, BOTH_SABER_IDLE_STANCE, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
									}
								}
							}
						}
...
qboolean PM_InOnGroundAnims(int anim)
{
	switch (anim)
	{
	case BOTH_DEAD1:
...
qboolean PM_FaceProtectAnim(int anim)
...
qboolean BG_FullBodyEmoteAnim(int anim)
{
	switch (anim)
	{
...
qboolean BG_FullBodyRespectAnim(int anim)
{
	switch (anim)
	{
	case BOTH_BOW:
...
qboolean BG_FullBodyCowerstartAnim(int anim)
{
	switch (anim)
	{
	case BOTH_COWER1:
...
qboolean BG_FullBodyCowerAnim(int anim)
{
	switch (anim)
	{
	case BOTH_COWER1:
...
qboolean BG_IsAlreadyinTauntAnim(int anim)
{
	switch (anim)
	{
	case BOTH_GESTURE1:
...
qboolean BG_AnimRequiresResponce(int anim)
{
	switch (anim)
	{
	case BOTH_GESTURE1:
...
qboolean BG_AnimIsSurrenderingandRequiresResponce(int anim)
{
	switch (anim)
	{
	case PLAYER_SURRENDER_START:
