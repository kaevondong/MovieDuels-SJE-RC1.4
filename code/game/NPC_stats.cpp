...
======================
*/
qboolean G_ParseAnimationFile_MD(int glaIndex, const char* skeletonName, int fileIndex)
{
	char			text[160000];
	int				len = 0;
	const char* token = 0;
				...
				{//backwards
...
#ifdef __SABER_ANIMATION_BLEND__
					{//Slow down saber moves...
						...
						for (x = 4; x < LS_MOVE_MAX; x++)
						{
...
#ifdef __SABER_SPINAROUND_RESTRICT__
							if (saberMoveData[x].animToUse + (77 * 2) == animNum)// SS_STRONG
							{
								animations[animNum].frameLerp *= 1.3;
								break;
							}
							else if (saberMoveData[x].animToUse + (77 * 3) == animNum) // SS_DESANN
							{
								animations[animNum].frameLerp *= 1.3;
								break;
							}
							...
							else if (saberMoveData[x].animToUse + (77 * 6) == animNum) // SS_STAFF
							{
								animations[animNum].frameLerp *= 1.1;
								break;
							}
...
//			animations[i].initialLerp	= 100;
		...
		if (Q_stricmp(skeletonName, "_humanoid")==0)
		{
			...
			{
				G_ParseAnimationFile_MD(0, skeletonName, fileIndex);
			}
			else
			{
			...
			if (cineGLAIndex)
			{
				...
				if (g_SaberAttackSpeedMD->integer)
				{
					G_ParseAnimationFile_MD(1, skeletonMapName, fileIndex);
				}
				else
				{
...
void CG_NPC_Precache ( gentity_t *spawner )
{
	...
	while ( 1 )
	{
		...
		if (!Q_stricmp(token, "friendlyfaction"))
		{
			...
			friendlyfaction = (faction_t)GetIDForString(FactionTable, token);
			continue;
		}
	...
	//===AI STATS=====================================================================
		...
		//		}
			...
			if (!Q_stricmp(token, "friendlyfaction"))
			{
				...
				NPC->client->friendlyfaction = (faction_t)GetIDForString(FactionTable, value);
				continue;
			}
			...
			if (!Q_stricmp(token, "enemyfaction"))
			{
				...
				NPC->client->enemyfaction = (faction_t)GetIDForString(FactionTable, value);
				continue;
			}
