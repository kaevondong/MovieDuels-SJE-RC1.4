...
void G_AddMercBalance(gentity_t* self, int amount)
{
	...
	if (self->s.clientNum >= MAX_CLIENTS && !G_ControlledByPlayer(self))
	{
		return;
	}
	...
	self->client->ps.BlasterAttackChainCount += amount;

	if (self->client->ps.BlasterAttackChainCount < BLASTERMISHAPLEVEL_NONE)
	{
		self->client->ps.BlasterAttackChainCount = BLASTERMISHAPLEVEL_NONE;
	}
	else if (self->client->ps.BlasterAttackChainCount > BLASTERMISHAPLEVEL_MAX)
	{
		self->client->ps.BlasterAttackChainCount = BLASTERMISHAPLEVEL_MAX;
	}
}
...
//---------------------------------------------------------
{
	...
	if ( ent->s.weapon == WP_DISRUPTOR && alt_fire )
	{
		...
		G_AddMercBalance(ent, BLASTERMISHAPLEVEL_LIGHT);
	}
	...
	else
	{
		...
		else
		{
			...
			{
				G_AddMercBalance(ent, BLASTERMISHAPLEVEL_MEDIUM);
			}
			else
			{
				if (ent->client->pers.cmd.forwardmove == 0 && ent->client->pers.cmd.rightmove == 0)
				{
					G_AddMercBalance(ent, Q_irand(BLASTERMISHAPLEVEL_MIN, BLASTERMISHAPLEVEL_RUNINACCURACY));// 1 was not enough
				}
				else
				{
					G_AddMercBalance(ent, Q_irand(BLASTERMISHAPLEVEL_MININACCURACY, BLASTERMISHAPLEVEL_MAXINACCURACY));// 1 was not enough
				}
			}
		}
	}
