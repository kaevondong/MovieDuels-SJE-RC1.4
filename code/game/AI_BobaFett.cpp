...
void Boba_FireFlameThrower(gentity_t* self)
{
	...
	int			damage_md = BOBA_FLAMETHROWDAMAGEMIN;
	...
	if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage)
	{
		...
		{
			G_Damage(traceEnt, self, self, dir, tr.endpos, damage_md, DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC | DAMAGE_IGNORE_TEAM, MOD_LAVA, HL_NONE);
		}
		else
		{
		...
		if (traceEnt->health > 0 && traceEnt->painDebounceTime > level.time)
		{
			...
			{
				if (g_SerenityJediEngineMode->integer == 2 && traceEnt->health < 30 && traceEnt->ghoul2.size())
				{
					if (traceEnt->chestBolt != -1)
					{
						G_PlayEffect(G_EffectIndex("flamethrower/flame_impact"), traceEnt->playerModel, traceEnt->chestBolt, traceEnt->s.number, traceEnt->currentOrigin, 3000, qtrue);
					}
				}
		...
		case WP_BOBA:
			...
			if (Distance(NPC->currentOrigin, NPC->enemy->currentOrigin) > 400.0f)
			{
				if (TIMER_Done(NPC, "nextBlasterAltFireDecide"))
				{
					if (Q_irand(0, (NPC->count * 2) + 3) > 2)
					{
						TIMER_Set(NPC, "nextBlasterAltFireDecide", Q_irand(6000, 8000));
						if (!(NPCInfo->scriptFlags & SCF_ALT_FIRE))
						{
							Boba_Printf("ALT FIRE On");
							NPCInfo->scriptFlags |= SCF_ALT_FIRE;
					...
					{
						TIMER_Set(NPC, "nextBlasterAltFireDecide", Q_irand(2000, 3000));
						if ((NPCInfo->scriptFlags & SCF_ALT_FIRE))
						{
							Boba_Printf("ALT FIRE Off");
							NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
